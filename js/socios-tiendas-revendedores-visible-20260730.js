(() => {
  ...
  async function cambiarEstadoTienda(id) {
    if (modulo !== "REVENDEDOR") return;

    const registro = estado.registros.find((item) => item.id === id);
    if (!registro) return;

    const estaActiva = String(registro.tiendaActiva || "SI").toUpperCase() !== "NO";
    const nuevoEstado = estaActiva ? "NO" : "SI";
    const accionTexto = estaActiva ? "apagar" : "activar";

    if (!window.confirm(
      `¿Deseas ${accionTexto} la tienda de ${registro.nombre} ${registro.apellido}?`
    )) return;

    try {
      if (tieneConexionApi()) {
        await peticionApi("cambiarEstadoTiendaRevendedorMD20", {
          id,
          activa: nuevoEstado
        });
      } else {
        const locales = leerLocal().map((item) =>
          item.id === id
            ? { ...item, tiendaActiva: nuevoEstado }
            : item
        );
        guardarLocal(locales);
      }
      ...
      mostrarMensaje(
        nuevoEstado === "SI"
          ? "La tienda del revendedor fue activada."
          : "La tienda del revendedor fue apagada. Su enlace público ya no mostrará productos.",
        "exito"
      );
    } catch (error) {
      mostrarMensaje(
      ...
      fila.innerHTML = `
        <td>
          <div class="celda-contacto">
            ...
            <div class="contacto-con-tienda">
              <strong>${escaparHtml(registro.nombre)} ${escaparHtml(registro.apellido)}</strong>
              <small>${escaparHtml(registro.whatsapp)}</small>

              ${modulo === "REVENDEDOR" ? `
                <button
                  type="button"
                  class="control-tienda-visible ${String(registro.tiendaActiva || "SI").toUpperCase() === "NO" ? "apagada" : "activa"}"
                  data-accion="tienda"
                  data-id="${escaparHtml(registro.id)}"
                  title="${String(registro.tiendaActiva || "SI").toUpperCase() === "NO" ? "Activar tienda" : "Apagar tienda"}"
                >
                  <svg><use href="#i-store"></use></svg>
                  <span>
                    ${String(registro.tiendaActiva || "SI").toUpperCase() === "NO"
                      ? "Tienda apagada"
                      : "Tienda activa"}
                  </span>
                </button>
  ...
  function manejarAccionTabla(evento) {
    ...
    if (accion === "tienda") cambiarEstadoTienda(id);
