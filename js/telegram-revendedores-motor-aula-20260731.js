...
async function accionFila(accion,id){
  try{
    if(accion==="enlace"){
      ...
      const r=d.registro||{};
      enlaceActual=r.comando||"";
      enlaceBotActual=r.enlaceBot||"";
...
async function diagnosticarWebhook(){
  try{
    const d=await get("diagnosticarWebhookTelegramRevendedoresMD20");
    const r=d.registro||{};
    const t=r.telegram||{};

    E.diagnostico.innerHTML=`
      <strong>${r.coincide?"✅ Webhook correcto":"❌ Webhook incorrecto"}</strong>
      <span><b>URL guardada:</b> ${t.url||"Vacía"}</span>
      <span><b>Actualizaciones pendientes:</b> ${Number(t.actualizacionesPendientes||0)}</span>
      <span><b>Último error:</b> ${t.ultimoErrorMensaje||"Sin errores"}</span>
      <span><b>Fecha del error:</b> ${t.ultimoErrorFecha||"—"}</span>
    `;
  }catch(e){
    E.diagnostico.innerHTML=`<strong>Diagnóstico fallido</strong><span>${e.message}</span>`;
  }
}
...
E.cerrar.addEventListener("click",(evento)=>{
...
E.modal.addEventListener("click",(evento)=>{
  if(evento.target===E.modal){
    cerrarModalEnlace();
  }
});

document.addEventListener("keydown",(evento)=>{
  if(evento.key==="Escape"&&!E.modal.hidden){
    cerrarModalEnlace();
  }
});
E.copiar.addEventListener("click",async()=>{
  if(!enlaceActual){
    mensaje("Primero configura el bot y genera un código válido.");
    return;
  }

  try{
    await navigator.clipboard.writeText(enlaceActual);
    mensaje("Comando copiado.");
  }catch(error){
    E.enlaceModal.focus();
    E.enlaceModal.select();
...
E.abrir.addEventListener("click",()=>{
  ...
  window.open(enlaceBotActual||"https://t.me/","_blank","noopener");
});

Promise.all([cargarConfiguracion(),cargarLista(),diagnosticarWebhook()]).catch(e=>mensaje(e.message));
})();