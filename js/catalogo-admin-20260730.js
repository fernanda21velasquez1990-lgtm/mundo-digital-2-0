...
console.info("Mundo Digital 2.0 - Catálogo administrativo build 2026-07-30-001");
...
function tarjetaProducto(producto){
  const imagen=producto.imagenUrl
    ? `style="background-image:linear-gradient(rgba(7,7,10,.12),rgba(7,7,10,.38)),url('${escAttr(producto.imagenUrl)}')"`
    : "";
  ...
  return `
    <article class="tarjeta-catalogo">
      <div class="imagen-catalogo" ${imagen}>
        ${producto.imagenUrl?"":'<svg><use href="#i-box"></use></svg>'}
...
function actualizarVistaImagen(){
  const url=C.imagenUrl.value.trim();

  if(url){
    E.vistaImagen.style.backgroundImage=`url("${url.replace(/"/g,"%22")}")`;
    E.vistaImagen.innerHTML="";
  }else{
    E.vistaImagen.style.backgroundImage="";
