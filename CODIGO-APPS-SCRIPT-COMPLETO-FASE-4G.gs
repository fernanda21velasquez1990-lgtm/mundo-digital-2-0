...
/*
 * FASE 4E — COMISIÓN AUTOMÁTICA POR VENTA PAGADA
 * Añade generación idempotente de comisiones para VENDEDORES.
 */
/**
 * FASE 4D: corrige validación y atribución de vendedor en tienda pública.
 * =========================================================
...
function doGet(e) {
  try {
    ...
    if(a==='listarUsuariosAdminMD20')return respuestaJson_(listarUsuariosAdminMD20_());
    if(a==='listarConfiguracionAdminMD20')return respuestaJson_(listarConfiguracionAdminMD20_());
...
function doPost(e) {
  try {
    ...
    if(a==='guardarUsuarioAdminMD20')return respuestaJson_({ok:true,registro:guardarUsuarioAdminMD20_(c.registro||{})});
    if(a==='cambiarEstadoUsuarioAdminMD20')return respuestaJson_({ok:true,registro:cambiarEstadoUsuarioAdminMD20_(c.usuarioId||'',c.estado||'')});
    if(a==='guardarConfiguracionAdminMD20')return respuestaJson_(guardarConfiguracionAdminMD20_(c.registros||[]));
...
/**
 * Normaliza el origen del cliente para respetar la validación de la columna K.
 * También corrige valores antiguos de la tienda como TIENDA_PUBLICA/TIENDA_WEB.
 */
function normalizarOrigenClienteMD20_(valor) {
  const limpio=String(valor||'WHATSAPP').trim().toUpperCase();
  const permitidos=['WHATSAPP','INSTAGRAM','FACEBOOK','TELEGRAM','PAGINA_WEB','REFERIDO','OTRO'];

  if(permitidos.includes(limpio))return limpio;

  const aliasWeb=['TIENDA_PUBLICA','TIENDA_WEB','WEB','PAGINA','PAGINA TIENDA','CATALOGO_WEB','CHECKOUT_WEB'];
  if(aliasWeb.includes(limpio))return 'PAGINA_WEB';
...
function guardarClienteMD20_(registro) {
  ...
  const fila=[
    ...
    normalizarOrigenClienteMD20_(registro.origenCliente),
...
function cambiarEstadoPagoVentaMD20_(ventaId,estado){
  ...
  try{
    ...
    if(estado==='PAGADO'){
      try{
        asegurarComisionVentaVendedorMD20_(ventaId);
      }catch(errorComision){
        console.error('No se pudo asegurar la comisión de '+ventaId+': '+errorComision.message);
        try{registrarLogVentaMD20_('ERROR_COMISION_VENTA',ventaId,errorComision.message||'Error al generar comisión.');}catch(_e){}
      }
    }
...
function recalcularVentaPorPagosMD20_(ventaId){
  ...
  // FASE 4E: una venta pagada atribuida a un vendedor debe generar
  // exactamente una comisión. La función es idempotente y no duplica.
...
/**
 * =========================================================
 * MUNDO DIGITAL 2.0 — FASE 4E
 * COMISIONES AUTOMÁTICAS DE VENTAS DE VENDEDORES
 * =========================================================
 *
 * - Se ejecuta automáticamente cuando una venta queda PAGADA.
 * - Usa VENTAS.VENDEDOR_ID para identificar al vendedor.
 * - Lee COMISION_TIPO y COMISION_VALOR desde VENDEDORES.
 * - No genera comisión para ADMIN/ADMINISTRADOR.
 * - Es idempotente: una misma venta nunca crea dos comisiones.
 * - Conserva compatibilidad con la hoja de comisiones ya usada
 *   por el Bot Admin y por el Panel del Vendedor.
 */
function asegurarComisionVentaVendedorMD20_(ventaId){
  ventaId=String(ventaId||'').trim();
  if(!ventaId)return null;

  const libro=md20LibroEstable_();
  const hojaVentas=libro.getSheetByName('VENTAS');
  if(!hojaVentas||hojaVentas.getLastRow()<=1)return null;

  const hv=hojaVentas.getRange(1,1,1,hojaVentas.getLastColumn())
    .getDisplayValues()[0]
    .map(v=>String(v||'').trim().toUpperCase());
  const cv=n=>hv.indexOf(n);

  const iVenta=cv('VENTA_ID');
  const iNumero=cv('NUMERO_VENTA');
  const iCliente=cv('CLIENTE_ID');
  const iTotal=cv('TOTAL');
  const iMoneda=cv('MONEDA');
  const iPago=cv('ESTADO_PAGO');
  const iVendedor=cv('VENDEDOR_ID');

  if(iVenta<0||iTotal<0||iPago<0||iVendedor<0)return null;

  const filasVentas=hojaVentas
    .getRange(2,1,hojaVentas.getLastRow()-1,hojaVentas.getLastColumn())
    .getValues();
  const fv=filasVentas.find(f=>String(f[iVenta]||'').trim()===ventaId);
  if(!fv)return null;

  if(String(fv[iPago]||'').trim().toUpperCase()!=='PAGADO')return null;

  const vendedorId=String(fv[iVendedor]||'').trim();
  const vendedorUpper=vendedorId.toUpperCase();
  if(!vendedorId||['ADMIN','ADMINISTRADOR','TIENDA_PUBLICA'].includes(vendedorUpper))return null;

  const total=Number(fv[iTotal]||0);
  if(!Number.isFinite(total)||total<=0)return null;

  const numeroVenta=iNumero>=0?String(fv[iNumero]||'').trim():'';
  const clienteId=iCliente>=0?String(fv[iCliente]||'').trim():'';
  const moneda=iMoneda>=0?String(fv[iMoneda]||'USD').trim().toUpperCase()||'USD':'USD';

  // Datos del vendedor y configuración de comisión.
  const hojaVendedores=libro.getSheetByName('VENDEDORES');
  if(!hojaVendedores||hojaVendedores.getLastRow()<=1)return null;

  const hvend=hojaVendedores.getRange(1,1,1,hojaVendedores.getLastColumn())
    .getDisplayValues()[0]
    .map(v=>String(v||'').trim().toUpperCase());
  const iv=n=>hvend.indexOf(n);
  const ivId=iv('VENDEDOR_ID');
  if(ivId<0)return null;

  const filasVendedores=hojaVendedores
    .getRange(2,1,hojaVendedores.getLastRow()-1,hojaVendedores.getLastColumn())
    .getValues();
  const fVend=filasVendedores.find(f=>String(f[ivId]||'').trim()===vendedorId);
  if(!fVend)return null;

  const estadoVend=iv('ESTADO')>=0?String(fVend[iv('ESTADO')]||'ACTIVO').trim().toUpperCase():'ACTIVO';
  if(estadoVend!=='ACTIVO')return null;

  const vendedorNombre=iv('NOMBRE_COMPLETO')>=0
    ? String(fVend[iv('NOMBRE_COMPLETO')]||vendedorId).trim()
    : vendedorId;
  const tipo=iv('COMISION_TIPO')>=0
    ? String(fVend[iv('COMISION_TIPO')]||'PORCENTAJE').trim().toUpperCase()
    : 'PORCENTAJE';
  const valor=iv('COMISION_VALOR')>=0
    ? Number(fVend[iv('COMISION_VALOR')]||0)
    : 0;

  if(!Number.isFinite(valor)||valor<=0)return null;

  let monto=0;
  if(['FIJO','FIJA','MONTO','MONTO_FIJO'].includes(tipo)){
    monto=valor;
  }else{
    monto=total*(valor/100);
  }
  monto=Math.round((monto+Number.EPSILON)*100)/100;
  if(monto<=0)return null;

  // Nombre del cliente para que el panel y el bot muestren el detalle.
  let clienteNombre='';
  if(clienteId){
    try{
      const cliente=listarClientesMD20_().find(c=>String(c.id||'').trim()===clienteId);
      if(cliente)clienteNombre=String(cliente.nombreCompleto||'').trim();
    }catch(_e){}
  }

  const hojaCom=obtenerOCrearHojaComisionesVentasMD20_(libro);
  const headers=hojaCom.getRange(1,1,1,hojaCom.getLastColumn())
    .getDisplayValues()[0]
    .map(v=>String(v||'').trim().toUpperCase());

  const idx=aliases=>{
    for(let i=0;i<aliases.length;i++){
      const p=headers.indexOf(aliases[i]);
      if(p>=0)return p;
    }
    return -1;
  };

  const iComVenta=idx(['VENTA_ID','REGISTRO_ORIGEN_ID','ORIGEN_ID']);
  const iComNumero=idx(['NUMERO_VENTA','VENTA_NUMERO']);
  const iComNotas=idx(['NOTAS','OBSERVACIONES']);

  // Protección doble: referencia directa y también texto en NOTAS.
  if(hojaCom.getLastRow()>1){
    const existentes=hojaCom
      .getRange(2,1,hojaCom.getLastRow()-1,hojaCom.getLastColumn())
      .getDisplayValues();

    const duplicada=existentes.some(f=>{
      if(iComVenta>=0&&String(f[iComVenta]||'').trim()===ventaId)return true;
      if(numeroVenta&&iComNumero>=0&&String(f[iComNumero]||'').trim()===numeroVenta)return true;
      const notas=iComNotas>=0?String(f[iComNotas]||''):'';
      return notas.indexOf(ventaId)>=0||(numeroVenta&&notas.indexOf(numeroVenta)>=0);
    });
  ...
  const ahora=new Date();
  const comisionId='COM-VEN-'+Utilities.getUuid().replace(/-/g,'').slice(0,10).toUpperCase();
  const nueva=new Array(headers.length).fill('');

  const poner=(aliases,valorCampo)=>{
    const p=idx(aliases);
    if(p>=0)nueva[p]=valorCampo;
  };

  poner(['COMISION_ID','ID_COMISION','REGISTRO_ID'],comisionId);
  poner(['VENDEDOR_ID','ID_VENDEDOR'],vendedorId);
  poner(['VENDEDOR_NOMBRE','NOMBRE_VENDEDOR','VENDEDOR'],vendedorNombre);
  poner(['CLIENTE_ID','ID_CLIENTE','CLIENTE_CODIGO'],clienteId);
  poner(['CLIENTE','CLIENTE_NOMBRE','NOMBRE_CLIENTE','CLIENTE_NOMBRE_COMPLETO','NOMBRE_COMPLETO_CLIENTE'],clienteNombre);
  poner(['VENTA_ID','REGISTRO_ORIGEN_ID','ORIGEN_ID'],ventaId);
  poner(['NUMERO_VENTA','VENTA_NUMERO'],numeroVenta);
  poner(['MONTO_VENTA','TOTAL_VENTA','VENTA_MONTO'],total);
  poner(['COMISION_TIPO','TIPO_COMISION','TIPO'],tipo||'PORCENTAJE');
  poner(['COMISION_VALOR','VALOR_COMISION','PORCENTAJE','VALOR'],valor);
  poner(['COMISION_MONTO','MONTO_COMISION','COMISION','IMPORTE_COMISION'],monto);
  poner(['MONEDA'],moneda);
  poner(['ESTADO_COMISION','ESTADO'],'PENDIENTE');
  poner(['GENERADA_EN','GENERADO_EN','CREADO_EN','FECHA_GENERADA'],ahora);
  poner(['ACTUALIZADO_EN'],ahora);
  poner(['ORIGEN','TIPO_ORIGEN'],'VENTA');

  const nota=[
    'Comisión automática por venta pagada.',
    'VENTA_ID:'+ventaId,
    numeroVenta?'NUMERO_VENTA:'+numeroVenta:'',
    clienteNombre?'CLIENTE:'+clienteNombre:''
  ].filter(Boolean).join(' | ');
  poner(['NOTAS','OBSERVACIONES'],nota);

  hojaCom.appendRow(nueva);
  const filaNueva=hojaCom.getLastRow();

  // Formatos básicos sin alterar el diseño ya existente de la hoja.
  [
    ['MONTO_VENTA','TOTAL_VENTA','VENTA_MONTO'],
    ['COMISION_MONTO','MONTO_COMISION','COMISION','IMPORTE_COMISION']
  ].forEach(aliases=>{
    const p=idx(aliases);
    if(p>=0)hojaCom.getRange(filaNueva,p+1).setNumberFormat('#,##0.00');
  });

  [
    ['GENERADA_EN','GENERADO_EN','CREADO_EN','FECHA_GENERADA'],
    ['PAGADO_EN','PAGADA_EN','FECHA_PAGO'],
    ['ACTUALIZADO_EN']
  ].forEach(aliases=>{
    const p=idx(aliases);
    if(p>=0)hojaCom.getRange(filaNueva,p+1).setNumberFormat('dd/MM/yyyy HH:mm');
  });

  SpreadsheetApp.flush();
  try{
    registrarLogVentaMD20_(
      'GENERAR_COMISION_VENTA',
      ventaId,
      'Comisión '+comisionId+' creada para '+vendedorId+' por '+monto.toFixed(2)+' '+moneda+'.'
    );
  }catch(_e){}
...
function obtenerOCrearHojaComisionesVentasMD20_(libro){
  ...
  hoja=libro.insertSheet('COMISIONES_VENDEDORES');
  const headers=[
    'COMISION_ID','VENDEDOR_ID','VENDEDOR_NOMBRE','CLIENTE_ID','CLIENTE_NOMBRE',
    'VENTA_ID','NUMERO_VENTA','MONTO_VENTA','COMISION_TIPO','COMISION_VALOR',
    'COMISION_MONTO','MONEDA','ESTADO_COMISION','GENERADA_EN','PAGADO_EN',
...
/**
 * Ejecutar UNA SOLA VEZ después de instalar Fase 4E.
 * Revisa ventas ya pagadas y crea solamente las comisiones faltantes.
 * Es seguro repetirla porque no duplica una venta ya comisionada.
 */
function SINCRONIZAR_COMISIONES_VENTAS_PAGADAS_MD20(){
  const libro=md20LibroEstable_();
  const hoja=libro.getSheetByName('VENTAS');
  if(!hoja||hoja.getLastRow()<=1){
    return {ok:true,revisadas:0,creadas:0,omitidas:0,errores:[]};
  }

  const headers=hoja.getRange(1,1,1,hoja.getLastColumn()).getDisplayValues()[0]
    .map(v=>String(v||'').trim().toUpperCase());
  const iId=headers.indexOf('VENTA_ID');
  const iPago=headers.indexOf('ESTADO_PAGO');
  const iVend=headers.indexOf('VENDEDOR_ID');
  if(iId<0||iPago<0||iVend<0)throw new Error('VENTAS no tiene VENTA_ID, ESTADO_PAGO o VENDEDOR_ID.');

  const filas=hoja.getRange(2,1,hoja.getLastRow()-1,hoja.getLastColumn()).getValues();
  let revisadas=0,creadas=0,omitidas=0;
  const errores=[];

  filas.forEach(f=>{
    const ventaId=String(f[iId]||'').trim();
    const estado=String(f[iPago]||'').trim().toUpperCase();
    const vendedor=String(f[iVend]||'').trim();
    if(!ventaId||estado!=='PAGADO'||!vendedor||['ADMIN','ADMINISTRADOR','TIENDA_PUBLICA'].includes(vendedor.toUpperCase()))return;
    revisadas++;
    try{
      const creada=asegurarComisionVentaVendedorMD20_(ventaId);
      if(creada)creadas++;else omitidas++;
    }catch(error){
      errores.push(ventaId+': '+(error.message||error));
    }
  });

  SpreadsheetApp.flush();
  const resultado={ok:errores.length===0,revisadas:revisadas,creadas:creadas,omitidas:omitidas,errores:errores};
  try{
    SpreadsheetApp.getUi().alert(
      'Comisiones sincronizadas',
      'Ventas revisadas: '+revisadas+'\n'+
      'Comisiones creadas: '+creadas+'\n'+
...
function crearVentaDesdePedidoTiendaMD20_(datos){
  ...
  const vendedorIdVenta=datos.vendedor==='ADMIN'
    ? 'ADMINISTRADOR'
    : String(datos.vendedor||'').trim();
...
function validarVendedorPedidoTiendaMD20_(valor){
  ...
  const vendedorUpper=vendedor.toUpperCase();

  if(
    !vendedor||
    ['ADMIN','ADMINISTRADOR'].includes(vendedorUpper)
  ){
    return 'ADMIN';
  }
  ...
  // ---------------------------------------------------------
  // 1) VENDEDORES WEB (VND-...)
  // ---------------------------------------------------------
  const hojaVendedores=libro.getSheetByName('VENDEDORES');

  if(hojaVendedores&&hojaVendedores.getLastRow()>1){
    const encabezados=hojaVendedores
      .getRange(1,1,1,hojaVendedores.getLastColumn())
      .getDisplayValues()[0]
      .map(v=>String(v||'').trim().toUpperCase());

    const colId=encabezados.indexOf('VENDEDOR_ID');
    const colEstado=encabezados.indexOf('ESTADO');
    const colTiendaActiva=encabezados.indexOf('TIENDA_ACTIVA');

    if(colId>=0){
      const fila=hojaVendedores
        .getRange(2,1,hojaVendedores.getLastRow()-1,hojaVendedores.getLastColumn())
        .getDisplayValues()
        .find(f=>String(f[colId]||'').trim().toUpperCase()===vendedorUpper);

      if(fila){
        const estado=colEstado>=0
          ? String(fila[colEstado]||'ACTIVO').trim().toUpperCase()
          : 'ACTIVO';
        ...
        if(colTiendaActiva>=0){
          const tiendaActiva=String(fila[colTiendaActiva]||'NO').trim().toUpperCase();
          if(tiendaActiva!=='SI'){
            throw new Error('La tienda de este vendedor no está activa.');
          }
        }

        return String(fila[colId]||vendedor).trim();
      }
    }
  }

  // ---------------------------------------------------------
  // 2) Compatibilidad con enlaces antiguos de REVENDEDORES
  // ---------------------------------------------------------
  const hojaRevendedores=libro.getSheetByName('REVENDEDORES');

  if(hojaRevendedores&&hojaRevendedores.getLastRow()>1){
    const encabezados=hojaRevendedores
      .getRange(1,1,1,hojaRevendedores.getLastColumn())
      .getDisplayValues()[0]
      .map(v=>String(v||'').trim().toUpperCase());
    ...
    if(colId>=0){
      const fila=hojaRevendedores
        .getRange(2,1,hojaRevendedores.getLastRow()-1,hojaRevendedores.getLastColumn())
        .getDisplayValues()
        .find(f=>String(f[colId]||'').trim().toUpperCase()===vendedorUpper);
...
/**
 * =========================================================
 * MUNDO DIGITAL 2.0 — FASE 4F
 * ADMINISTRACIÓN WEB DE USUARIOS Y CONFIGURACIÓN
 * =========================================================
 */
function listarUsuariosAdminMD20_(){
  const libro=md20LibroEstable_();
  const hoja=libro.getSheetByName('USUARIOS');
  if(!hoja)return {ok:true,registros:[],roles:[],vendedores:[]};
  asegurarColumnaVendedorIdUsuariosMD20_(hoja);
  const headers=hoja.getRange(1,1,1,hoja.getLastColumn()).getDisplayValues()[0].map(v=>String(v||'').trim().toUpperCase());
  const idx=n=>headers.indexOf(n);
  const filas=hoja.getLastRow()>1?hoja.getRange(2,1,hoja.getLastRow()-1,hoja.getLastColumn()).getValues():[];
  const registros=filas.filter(f=>String(f[idx('USUARIO_ID')]||'').trim()).map(f=>({
    id:String(f[idx('USUARIO_ID')]||'').trim(),
    nombreCompleto:String(f[idx('NOMBRE_COMPLETO')]||'').trim(),
    usuario:String(f[idx('USUARIO')]||'').trim(),
    correo:String(f[idx('CORREO')]||'').trim(),
    rolId:String(f[idx('ROL_ID')]||'').trim(),
    telefono:String(f[idx('TELEFONO')]||'').trim(),
    estado:String(f[idx('ESTADO')]||'ACTIVO').trim().toUpperCase(),
    ultimoAcceso:idx('ULTIMO_ACCESO')>=0?fechaHoraApi_(f[idx('ULTIMO_ACCESO')]):'',
    creadoEn:idx('CREADO_EN')>=0?fechaHoraApi_(f[idx('CREADO_EN')]):'',
    actualizadoEn:idx('ACTUALIZADO_EN')>=0?fechaHoraApi_(f[idx('ACTUALIZADO_EN')]):'',
    vendedorId:idx('VENDEDOR_ID')>=0?String(f[idx('VENDEDOR_ID')]||'').trim():''
  }));

  const hojaRoles=libro.getSheetByName('ROLES_PERMISOS');
  let roles=[];
  if(hojaRoles&&hojaRoles.getLastRow()>1){
    const hr=hojaRoles.getRange(1,1,1,hojaRoles.getLastColumn()).getDisplayValues()[0].map(v=>String(v||'').trim().toUpperCase());
    const ir=n=>hr.indexOf(n);
    roles=hojaRoles.getRange(2,1,hojaRoles.getLastRow()-1,hojaRoles.getLastColumn()).getValues()
      .filter(f=>String(f[ir('ROL_ID')]||'').trim()&&String(f[ir('ESTADO')]||'ACTIVO').toUpperCase()!=='INACTIVO')
      .map(f=>({id:String(f[ir('ROL_ID')]||'').trim(),nombre:String(f[ir('NOMBRE')]||'').trim(),descripcion:String(f[ir('DESCRIPCION')]||'').trim()}));
  }

  const hojaVend=libro.getSheetByName('VENDEDORES');
  let vendedores=[];
  if(hojaVend&&hojaVend.getLastRow()>1){
    const hv=hojaVend.getRange(1,1,1,hojaVend.getLastColumn()).getDisplayValues()[0].map(v=>String(v||'').trim().toUpperCase());
    const iv=n=>hv.indexOf(n);
    if(iv('VENDEDOR_ID')>=0){
      vendedores=hojaVend.getRange(2,1,hojaVend.getLastRow()-1,hojaVend.getLastColumn()).getValues()
        .filter(f=>String(f[iv('VENDEDOR_ID')]||'').trim())
        .map(f=>({
          id:String(f[iv('VENDEDOR_ID')]||'').trim(),
          nombre:iv('NOMBRE_COMPLETO')>=0?String(f[iv('NOMBRE_COMPLETO')]||'').trim():'',
          usuario:iv('USUARIO')>=0?String(f[iv('USUARIO')]||'').trim():'',
          correo:iv('CORREO')>=0?String(f[iv('CORREO')]||'').trim():'',
          telefono:iv('WHATSAPP')>=0?String(f[iv('WHATSAPP')]||'').trim():'',
          estado:iv('ESTADO')>=0?String(f[iv('ESTADO')]||'ACTIVO').trim().toUpperCase():'ACTIVO'
        }));
    }
  }
  return {ok:true,registros:registros,roles:roles,vendedores:vendedores};
}

function guardarUsuarioAdminMD20_(r){
  const libro=md20LibroEstable_();
  const hoja=libro.getSheetByName('USUARIOS');
  ...
  const col=n=>headers.indexOf(n)+1;
  const idx=n=>headers.indexOf(n);
  const id=String(r.id||r.usuarioId||'').trim();
  const nombre=String(r.nombreCompleto||'').trim();
  const usuario=normalizarUsuario_(r.usuario||'');
  const correo=String(r.correo||'').trim().toLowerCase();
  const telefono=String(r.telefono||'').trim();
  const rol=String(r.rolId||'ROL-VENDEDOR').trim().toUpperCase();
  const estado=String(r.estado||'ACTIVO').trim().toUpperCase()==='INACTIVO'?'INACTIVO':'ACTIVO';
  const vendedorId=String(r.vendedorId||'').trim();
  const contrasena=String(r.contrasena||'');

  if(nombre.length<3)throw new Error('Escribe el nombre completo.');
  if(!validarNombreUsuario_(usuario))throw new Error('El usuario debe tener entre 3 y 30 caracteres y usar letras, números, puntos, guiones o guiones bajos.');
  if(correo&&!validarCorreo_(correo))throw new Error('Escribe un correo válido.');
  if(!['ROL-ADMIN','ROL-VENDEDOR','ROL-SOPORTE'].includes(rol))throw new Error('Selecciona un rol válido.');
  if(rol==='ROL-VENDEDOR'&&!vendedorId)throw new Error('Selecciona el vendedor asociado a este acceso.');

  let fila=0, anterior=null;
  if(id&&hoja.getLastRow()>1){
    const ids=hoja.getRange(2,col('USUARIO_ID'),hoja.getLastRow()-1,1).getDisplayValues().flat();
    const pos=ids.findIndex(v=>String(v||'').trim()===id);
    if(pos>=0){fila=pos+2;anterior=hoja.getRange(fila,1,1,hoja.getLastColumn()).getValues()[0];}
  }

  const registros=hoja.getLastRow()>1?hoja.getRange(2,1,hoja.getLastRow()-1,hoja.getLastColumn()).getValues():[];
  registros.forEach((f,i)=>{
    if(fila&&i+2===fila)return;
    if(String(f[idx('USUARIO')]||'').trim().toLowerCase()===usuario)throw new Error('Ya existe otro usuario con ese nombre de acceso.');
    if(correo&&String(f[idx('CORREO')]||'').trim().toLowerCase()===correo)throw new Error('Ya existe otro usuario con ese correo.');
    if(rol==='ROL-VENDEDOR'&&vendedorId&&idx('VENDEDOR_ID')>=0&&String(f[idx('VENDEDOR_ID')]||'').trim()===vendedorId)throw new Error('Ese vendedor ya tiene un usuario web asociado.');
  });

  const usuarioAnterior=anterior?String(anterior[idx('USUARIO')]||'').trim().toLowerCase():'';
  if(!fila&&!validarContrasena_(contrasena))throw new Error('Para crear el usuario, la contraseña debe tener mínimo 8 caracteres e incluir letras y números.');
  if(fila&&usuarioAnterior!==usuario&&!validarContrasena_(contrasena))throw new Error('Si cambias el nombre de usuario también debes escribir una contraseña nueva.');
  if(contrasena&&!validarContrasena_(contrasena))throw new Error('La contraseña debe tener mínimo 8 caracteres e incluir letras y números.');

  const ahora=new Date();
  const usuarioId=fila?id:generarSiguienteUsuarioId_(hoja);
  const claveHash=contrasena?generarHashContrasena_(usuario,contrasena):(anterior?anterior[idx('CLAVE_HASH')]:'');
  const nueva=new Array(hoja.getLastColumn()).fill('');
  const poner=(n,v)=>{const i=idx(n);if(i>=0)nueva[i]=v;};
  poner('USUARIO_ID',usuarioId); poner('NOMBRE_COMPLETO',nombre); poner('USUARIO',usuario); poner('CORREO',correo);
  poner('CLAVE_HASH',claveHash); poner('ROL_ID',rol); poner('TELEFONO',telefono); poner('ESTADO',estado);
  poner('ULTIMO_ACCESO',anterior&&idx('ULTIMO_ACCESO')>=0?anterior[idx('ULTIMO_ACCESO')]:'');
  poner('CREADO_EN',anterior&&idx('CREADO_EN')>=0&&anterior[idx('CREADO_EN')]?anterior[idx('CREADO_EN')]:ahora);
  poner('ACTUALIZADO_EN',ahora); poner('VENDEDOR_ID',rol==='ROL-VENDEDOR'?vendedorId:'');

  if(fila)hoja.getRange(fila,1,1,nueva.length).setValues([nueva]); else hoja.appendRow(nueva);
  SpreadsheetApp.flush();
  try{registrarLogAdministrador_(libro,usuarioId,fila?'ACTUALIZAR_USUARIO_WEB':'CREAR_USUARIO_WEB','Usuario '+usuario+' / '+rol+' guardado desde el panel.');}catch(_e){}
  return listarUsuariosAdminMD20_().registros.find(x=>x.id===usuarioId)||null;
}

function cambiarEstadoUsuarioAdminMD20_(usuarioId,estado){
  usuarioId=String(usuarioId||'').trim();
  estado=String(estado||'').trim().toUpperCase();
  if(!usuarioId)throw new Error('Falta el usuario.');
  if(!['ACTIVO','INACTIVO'].includes(estado))throw new Error('Estado no válido.');
  const libro=md20LibroEstable_(),hoja=libro.getSheetByName('USUARIOS');
  if(!hoja)throw new Error('No existe USUARIOS.');
  const h=hoja.getRange(1,1,1,hoja.getLastColumn()).getDisplayValues()[0].map(v=>String(v||'').trim().toUpperCase());
  const c=n=>h.indexOf(n)+1;
  const fila=buscarFilaPorValorMD20_(hoja,c('USUARIO_ID'),usuarioId);
  if(!fila)throw new Error('No se encontró el usuario.');
  const rol=String(hoja.getRange(fila,c('ROL_ID')).getValue()||'').toUpperCase();
  const activosAdmin=hoja.getLastRow()>1?hoja.getRange(2,1,hoja.getLastRow()-1,hoja.getLastColumn()).getValues().filter(f=>String(f[h.indexOf('ROL_ID')]||'').toUpperCase()==='ROL-ADMIN'&&String(f[h.indexOf('ESTADO')]||'').toUpperCase()==='ACTIVO').length:0;
  if(rol==='ROL-ADMIN'&&estado==='INACTIVO'&&activosAdmin<=1)throw new Error('No puedes desactivar el único administrador activo.');
  hoja.getRange(fila,c('ESTADO')).setValue(estado);
  if(c('ACTUALIZADO_EN'))hoja.getRange(fila,c('ACTUALIZADO_EN')).setValue(new Date());
  SpreadsheetApp.flush();
  try{registrarLogAdministrador_(libro,usuarioId,'CAMBIAR_ESTADO_USUARIO','Estado cambiado a '+estado+'.');}catch(_e){}
  return listarUsuariosAdminMD20_().registros.find(x=>x.id===usuarioId)||null;
}

function claveConfiguracionVisibleAdminMD20_(clave){
  const k=String(clave||'').trim().toUpperCase();
  if(!k)return false;
  const bloqueadas=['API_KEY_SOCIOS','SPREADSHEET_ID','URL_APPS_SCRIPT','CARPETA_COMPROBANTES_ID','CARPETA_PUBLICIDAD_ID'];
  if(bloqueadas.includes(k))return false;
  if(/SECRET|TOKEN|PASSWORD|CONTRASENA|CLAVE_PRIVADA|BOT_TOKEN/.test(k))return false;
  return true;
}


function asegurarPreferenciasVisualesAdminMD20_(){
  const libro=md20LibroEstable_();
  const hoja=libro.getSheetByName('CONFIGURACION');
  if(!hoja)return;

  const h=hoja.getRange(1,1,1,hoja.getLastColumn())
    .getDisplayValues()[0]
    .map(v=>String(v||'').trim().toUpperCase());
  const c=n=>h.indexOf(n)+1;
  if(!c('CLAVE')||!c('VALOR'))return;

  const existentes=hoja.getLastRow()>1
    ? hoja.getRange(2,c('CLAVE'),hoja.getLastRow()-1,1).getDisplayValues().flat()
      .map(v=>String(v||'').trim().toUpperCase())
    : [];

  const ahora=new Date();
  const preferencias=[
    ['TAMANO_LETRA_PANEL','GRANDE','Tamaño general de la letra del panel. Opciones: COMPACTO, NORMAL, GRANDE o MUY_GRANDE.','LISTA','SI',ahora],
    ['TIPO_LETRA_PANEL','SISTEMA','Tipo de letra del panel. Opciones: SISTEMA, ARIAL, VERDANA, TREBUCHET o GEORGIA.','LISTA','SI',ahora],
    ['MOSTRAR_EMOJIS_PANEL','SI','Permite mostrar u ocultar los emojis decorativos del panel.','SI_NO','SI',ahora],
    ['EMOJI_PANEL','✨','Emoji opcional que acompaña el título de cada sección del panel. Puede dejarse vacío.','TEXTO','SI',ahora]
  ];

  preferencias.forEach(fila=>{
    if(existentes.includes(String(fila[0]).toUpperCase()))return;
    hoja.appendRow(fila);
    existentes.push(String(fila[0]).toUpperCase());
  });
  SpreadsheetApp.flush();
}

function listarConfiguracionAdminMD20_(){
  asegurarPreferenciasVisualesAdminMD20_();
  const libro=md20LibroEstable_(),hoja=libro.getSheetByName('CONFIGURACION');
  if(!hoja||hoja.getLastRow()<=1)return {ok:true,registros:[]};
  const h=hoja.getRange(1,1,1,hoja.getLastColumn()).getDisplayValues()[0].map(v=>String(v||'').trim().toUpperCase());
  const i=n=>h.indexOf(n);
  const registros=hoja.getRange(2,1,hoja.getLastRow()-1,hoja.getLastColumn()).getValues().filter(f=>claveConfiguracionVisibleAdminMD20_(f[i('CLAVE')])).map(f=>({
    clave:String(f[i('CLAVE')]||'').trim(),
    valor:String(f[i('VALOR')]??''),
    descripcion:String(f[i('DESCRIPCION')]||'').trim(),
    tipo:String(f[i('TIPO')]||'TEXTO').trim().toUpperCase(),
    editable:String(f[i('EDITABLE')]||'NO').trim().toUpperCase()==='SI',
    actualizadoEn:i('ACTUALIZADO_EN')>=0?fechaHoraApi_(f[i('ACTUALIZADO_EN')]):''
  }));
  return {ok:true,registros:registros};
}

function guardarConfiguracionAdminMD20_(registros){
  asegurarPreferenciasVisualesAdminMD20_();
  if(!Array.isArray(registros))throw new Error('Configuración no válida.');
  const libro=md20LibroEstable_(),hoja=libro.getSheetByName('CONFIGURACION');
  if(!hoja)throw new Error('No existe CONFIGURACION.');
  const h=hoja.getRange(1,1,1,hoja.getLastColumn()).getDisplayValues()[0].map(v=>String(v||'').trim().toUpperCase());
  const c=n=>h.indexOf(n)+1;
  const filas=hoja.getLastRow()>1?hoja.getRange(2,1,hoja.getLastRow()-1,hoja.getLastColumn()).getValues():[];
  let guardadas=0;
  registros.forEach(r=>{
    ...
    if(!clave||!claveConfiguracionVisibleAdminMD20_(clave))return;
    const pos=filas.findIndex(f=>String(f[h.indexOf('CLAVE')]||'').trim().toUpperCase()===clave);
    if(pos<0)return;
    const fila=pos+2;
    const editable=String(filas[pos][h.indexOf('EDITABLE')]||'NO').trim().toUpperCase()==='SI';
    if(!editable)return;
    const tipo=String(filas[pos][h.indexOf('TIPO')]||'TEXTO').trim().toUpperCase();
    let valor=String(r.valor??'').trim();
    if(['BOOLEANO','SI_NO'].includes(tipo))valor=valor.toUpperCase()==='NO'?'NO':'SI';

    if(clave==='TAMANO_LETRA_PANEL'){
      valor=valor.toUpperCase();
      if(!['COMPACTO','NORMAL','GRANDE','MUY_GRANDE'].includes(valor))valor='GRANDE';
    }
    if(clave==='TIPO_LETRA_PANEL'){
      valor=valor.toUpperCase();
      if(!['SISTEMA','ARIAL','VERDANA','TREBUCHET','GEORGIA'].includes(valor))valor='SISTEMA';
    }
    if(clave==='MOSTRAR_EMOJIS_PANEL'){
      valor=valor.toUpperCase()==='NO'?'NO':'SI';
    }
    if(clave==='EMOJI_PANEL'){
      valor=Array.from(valor).slice(0,4).join('');
    }
    if(tipo==='NUMERO'&&valor!==''&&!Number.isFinite(Number(valor.replace(',','.'))))throw new Error('El valor de '+clave+' debe ser numérico.');
    if(tipo==='CORREO'&&valor&&!validarCorreo_(valor))throw new Error('El correo configurado no es válido.');
    hoja.getRange(fila,c('VALOR')).setValue(valor);
    if(c('ACTUALIZADO_EN'))hoja.getRange(fila,c('ACTUALIZADO_EN')).setValue(new Date());
    guardadas++;
  });
  SpreadsheetApp.flush();
  try{registrarLogAdministrador_(libro,'CONFIGURACION','ACTUALIZAR_CONFIGURACION_WEB','Se actualizaron '+guardadas+' ajustes desde el panel.');}catch(_e){}
  const respuesta=listarConfiguracionAdminMD20_();
  respuesta.guardadas=guardadas;
  return respuesta;
}
