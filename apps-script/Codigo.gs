...
function doGet(e) {
  try {
    ...
    // Portal nuevo: acceso por número de celular + token generado desde Telegram.
    if(a==='consultarPortalClienteLogin'){
      return respuestaJson_(consultarPortalClienteLoginMD20_(p.telefono||'',p.token||''));
    }
    ...
    if(a==='listarCatalogoPublicoMD20'){
      ...
      const acceso=obtenerAccesoTiendaRevendedorMD20_(canal);
      const registros=acceso.activa
        ? listarCatalogoPublicoMD20_(acceso.vendedor)
        : [];

      return respuestaJson_({
        ok:true,
        tiendaActiva:acceso.activa,
        mensajeTienda:acceso.mensaje,
        registros:registros,
    ...
    if(a==='obtenerConfiguracionTelegramPagosMD20')return respuestaJson_({
      ok:true,
      registro:obtenerConfiguracionTelegramPagosMD20_()
    });
    if(a==='obtenerConfiguracionTelegramRevendedoresMD20')return respuestaJson_({
      ok:true,
      registro:obtenerConfiguracionTelegramRevendedoresMD20_()
    });
    if(a==='listarVinculacionesTelegramRevendedoresMD20')return respuestaJson_({
      ok:true,
      registros:listarVinculacionesTelegramRevendedoresMD20_()
    });
    if(a==='diagnosticarWebhookTelegramRevendedoresMD20')return respuestaJson_({
      ok:true,
      registro:diagnosticarWebhookTelegramRevendedoresMD20_()
    });
...
function doPost(e) {
  try {
    ...
    if(c&&Object.prototype.hasOwnProperty.call(c,'update_id')){
      const parametros=e&&e.parameter?e.parameter:{};

      if(parametros.telegram_revendedores){
        return procesarWebhookTelegramRevendedoresMD20_(c,e);
      }

      return procesarWebhookTelegramPagosMD20_(c,e);
    }
    ...
    if(a==='guardarConfiguracionTelegramPagosMD20')return respuestaJson_({
      ok:true,
      registro:guardarConfiguracionTelegramPagosMD20_(c.registro||{})
    });
    if(a==='probarTelegramPagosMD20')return respuestaJson_(
      probarTelegramPagosMD20_()
    );
    if(a==='registrarWebhookTelegramPagosMD20')return respuestaJson_(
      registrarWebhookTelegramPagosMD20_()
    );
    if(a==='enviarAlertasTelegramPendientesMD20')return respuestaJson_(
      enviarAlertasTelegramPendientesMD20_()
    );
    if(a==='guardarConfiguracionTelegramRevendedoresMD20')return respuestaJson_({
      ok:true,
      registro:guardarConfiguracionTelegramRevendedoresMD20_(c.registro||{})
    });
    if(a==='probarTelegramRevendedoresMD20')return respuestaJson_(
      probarTelegramRevendedoresMD20_()
    );
    if(a==='registrarWebhookTelegramRevendedoresMD20')return respuestaJson_(
      registrarWebhookTelegramRevendedoresMD20_()
    );
    if(a==='generarEnlaceTelegramRevendedorMD20')return respuestaJson_({
      ok:true,
      registro:generarEnlaceTelegramRevendedorMD20_(c.revendedorId||'')
    });
    if(a==='desvincularTelegramRevendedorMD20')return respuestaJson_({
      ok:true,
      registro:desvincularTelegramRevendedorMD20_(c.revendedorId||'')
    });
    if(a==='enviarPruebaTelegramRevendedorMD20')return respuestaJson_(
      enviarPruebaTelegramRevendedorMD20_(c.revendedorId||'')
    );
    ...
    if(a==='cambiarEstadoTiendaRevendedorMD20')return respuestaJson_({
      ok:true,
      registro:cambiarEstadoTiendaRevendedorMD20_(c.id||'',c.activa)
    });
...
function listarSocios_(tipo) {
  if(tipo==='REVENDEDOR')asegurarControlTiendaRevendedoresMD20_();
...
function filaARegistro_(fila) {
  return {
    ...
    tiendaActiva: String(fila[20] || 'SI').toUpperCase() !== 'NO' ? 'SI' : 'NO'
  };
}
...
function guardarSocio_(tipo, registro) {
  ...
  const fila = [
    ...
    tipo === 'REVENDEDOR'
      ? normalizarSiNoTiendaRevendedorMD20_(
          registro.tiendaActiva ||
          (filaExistente ? hoja.getRange(filaExistente, 21).getDisplayValue() : 'SI')
        )
      : ''
  ];
...
function normalizarSiNoTiendaRevendedorMD20_(valor){
  return String(valor||'SI').trim().toUpperCase()==='NO'?'NO':'SI';
}

function asegurarControlTiendaRevendedoresMD20_(){
  const hoja=SpreadsheetApp.getActiveSpreadsheet().getSheetByName('REVENDEDORES');
  if(!hoja)return null;

  if(hoja.getMaxColumns()<21){
    hoja.insertColumnsAfter(hoja.getMaxColumns(),21-hoja.getMaxColumns());
  }

  hoja.getRange(1,21).setValue('TIENDA_ACTIVA');

  if(hoja.getLastRow()>1){
    const rango=hoja.getRange(2,21,hoja.getLastRow()-1,1);
    const valores=rango.getDisplayValues().map(fila=>[
      normalizarSiNoTiendaRevendedorMD20_(fila[0]||'SI')
    ]);
    rango.setValues(valores);
  }

  hoja.setColumnWidth(21,145);
  return hoja;
}

/**
 * Ejecutar una sola vez después de instalar esta actualización.
 */
function prepararControlTiendasRevendedoresMD20(){
  const hoja=asegurarControlTiendaRevendedoresMD20_();
  if(!hoja)throw new Error('No existe la pestaña REVENDEDORES.');

  SpreadsheetApp.flush();
  console.log('Control de tiendas de revendedores preparado correctamente.');

  return {
    ok:true,
    mensaje:'Control de tiendas de revendedores preparado correctamente.'
  };
}

function cambiarEstadoTiendaRevendedorMD20_(id,activa){
  const hoja=asegurarControlTiendaRevendedoresMD20_();
  if(!hoja)throw new Error('No existe la pestaña REVENDEDORES.');

  const fila=buscarFilaPorId_(hoja,String(id||'').trim());
  if(!fila)throw new Error('No se encontró el revendedor.');

  const nuevoEstado=
    activa===true||
    String(activa||'').trim().toUpperCase()==='SI'
      ? 'SI'
      : 'NO';

  hoja.getRange(fila,21).setValue(nuevoEstado);
  hoja.getRange(fila,20).setValue(new Date());
  SpreadsheetApp.flush();

  const datos=hoja
    .getRange(fila,1,1,MD20_SOCIOS.ENCABEZADOS.length)
    .getValues()[0];

  return filaARegistro_(datos);
}

function obtenerAccesoTiendaRevendedorMD20_(canal){
  const vendedor=String(canal||'ADMIN').trim();
  ...
  if(!hoja||hoja.getLastRow()<=1){
    return {
      vendedor:vendedor,
      activa:false,
      mensaje:'Esta tienda no está disponible.'
    };
  }

  const fila=buscarFilaPorId_(hoja,vendedor);

  if(!fila){
    return {
      vendedor:vendedor,
      activa:false,
      mensaje:'El enlace de esta tienda no es válido.'
    };
  }

  const estado=String(hoja.getRange(fila,15).getDisplayValue()||'ACTIVO').toUpperCase();
  const tienda=normalizarSiNoTiendaRevendedorMD20_(
    hoja.getRange(fila,21).getDisplayValue()||'SI'
  );
  ...
  if(estado==='INACTIVO'){
    return {
      vendedor:vendedor,
      activa:false,
      mensaje:'Este revendedor está inactivo.'
    };
  }
  ...
  if(tienda==='NO'){
    return {
      vendedor:vendedor,
      activa:false,
      mensaje:'Esta tienda fue desactivada temporalmente por el administrador.'
    };
  }
...
/**
 * PORTAL CLIENTE V2 — número de celular + token generado desde Telegram.
 * Mantiene el portal anterior para no romper enlaces viejos.
 */
function consultarPortalClienteLoginMD20_(telefono,token){
  const acceso=validarTokenClienteTelegramMD20_(telefono,token);
  const cliente=listarClientesMD20_().find(c=>c.id===acceso.clienteId);
  if(!cliente)throw new Error('El cliente asociado a este acceso no existe.');

  const servicios=[];

  listarSuscripcionesMD20_()
    .filter(s=>s.clienteId===cliente.id&&['ACTIVA','POR_VENCER','VENCIDA'].includes(String(s.estado||'').toUpperCase()))
    .forEach(s=>{
      servicios.push({
        id:s.id,
        tipoServicio:'SUSCRIPCION',
        nombre:s.productoNombre||s.plataforma||'Servicio digital',
        correo:s.usuarioCuenta||'',
        usuarioCuenta:s.usuarioCuenta||'',
        contrasenaCuenta:s.contrasenaCuenta||'',
        perfil:s.perfil||'',
        pin:s.pin||'',
        ...
        precioRenovacion:s.precioRenovacion,
        moneda:s.moneda,
        estado:s.estado,
        respuestaCliente:s.respuestaCliente||s.estadoRenovacion||'SIN_RESPUESTA'
        // Datos del proveedor se mantienen privados.
      });
    });

  listarCuentasCanvaMD20_()
    .filter(c=>(
      c.clienteId===cliente.id ||
      normalizarTelefonoPortalMD20_(c.whatsapp)===normalizarTelefonoPortalMD20_(cliente.telefono)
    )&&['ACTIVA','POR_VENCER','VENCIDA'].includes(String(c.estado||'').toUpperCase()))
    .forEach(c=>{
      servicios.push({
        id:c.id,
        tipoServicio:'CANVA',
        ...
        fechaInicio:c.fechaCompra,
        fechaVencimiento:c.fechaVencimiento,
        diasRestantes:c.diasRestantes,
        precioRenovacion:c.precio,
        moneda:c.moneda,
        estado:c.estado,
        respuestaCliente:c.respuestaCliente||'SIN_RESPUESTA'
      });
    });
...
function validarTokenClienteTelegramMD20_(telefono,token){
  ...
  if(!tel)throw new Error('Escribe el número de celular registrado.');
  if(!tok)throw new Error('Escribe tu token de acceso.');

  const h=SpreadsheetApp.getActiveSpreadsheet().getSheetByName('CLIENTES');
  if(!h||h.getLastRow()<=1)throw new Error('No hay clientes registrados.');

  const lastCol=h.getLastColumn();
  const headers=h.getRange(1,1,1,lastCol).getDisplayValues()[0].map(x=>String(x||'').trim().toUpperCase());

  const colId=buscarColumnaPortalMD20_(headers,['CLIENTE_ID','ID_CLIENTE','REGISTRO_ID','ID']);
  const colTel=buscarColumnaPortalMD20_(headers,['TELEFONO','WHATSAPP','CELULAR']);
  const colEstadoCliente=buscarColumnaPortalMD20_(headers,['ESTADO','ESTADO_CLIENTE','STATUS']);
  const colToken=buscarColumnaPortalMD20_(headers,['TOKEN']);
  const colTokenEstado=buscarColumnaPortalMD20_(headers,['TOKEN_ESTADO']);
  const colTokenVence=buscarColumnaPortalMD20_(headers,['TOKEN_VENCE']);

  if(colId<0||colTel<0||colToken<0){
    throw new Error('El acceso por token todavía no está preparado. Crea el token desde Telegram.');
  }

  const datos=h.getRange(2,1,h.getLastRow()-1,lastCol).getValues();
  const fila=datos.findIndex(r=>{
    const telefonoFila=normalizarTelefonoPortalMD20_(r[colTel]);
    const tokenFila=String(r[colToken]||'').trim().toUpperCase();
    return telefonoFila===tel && tokenFila===tok;
  });

  if(fila<0)throw new Error('Número de celular o token incorrecto.');

  const r=datos[fila];
  const estadoCliente=colEstadoCliente>=0?String(r[colEstadoCliente]||'ACTIVO').trim().toUpperCase():'ACTIVO';
  const estadoToken=colTokenEstado>=0?String(r[colTokenEstado]||'ACTIVO').trim().toUpperCase():'ACTIVO';

  if(['INACTIVO','BLOQUEADO','SUSPENDIDO'].includes(estadoCliente)){
    throw new Error('Tu cuenta de cliente está inactiva. Comunícate con soporte.');
  }
  if(estadoToken!=='ACTIVO'){
    throw new Error('Este token está inactivo. Comunícate con soporte.');
  }

  let vence=colTokenVence>=0?r[colTokenVence]:'';
  if(vence){
    const f=vence instanceof Date?vence:new Date(vence);
    if(!isNaN(f.getTime())&&f<new Date())throw new Error('Tu token de acceso ya venció. Comunícate con soporte para renovarlo.');
    vence=f;
  }

  return {
    clienteId:String(r[colId]||''),
    fila:fila+2,
    hoja:h,
    vence:vence
  };
...
function buscarColumnaPortalMD20_(headers,candidatos){
  for(let i=0;i<candidatos.length;i++){
    const p=headers.indexOf(String(candidatos[i]||'').trim().toUpperCase());
    if(p>=0)return p;
  }
  return -1;
}
...
function responderRenovacionPortalMD20_(c){
  const acceso=String(c.telefono||'').trim()
    ? validarTokenClienteTelegramMD20_(c.telefono||'',c.token||'')
    : validarTokenPortalMD20_(c.token||'');
  const respuesta=String(c.respuesta||'').toUpperCase();
...
function validarVendedorPedidoTiendaMD20_(valor){
  const acceso=obtenerAccesoTiendaRevendedorMD20_(valor||'ADMIN');

  if(!acceso.activa){
    throw new Error(acceso.mensaje||'Esta tienda no está disponible.');
  }

  return acceso.vendedor;
}
...
function registrarPagoPedidoTiendaMD20_(registro){
  ...
  try{
    enviarAlertaTelegramPedidoMD20_(pedido);
  }catch(errorTelegram){
    console.error(
      'El comprobante se registró, pero Telegram no pudo enviar la alerta: '+
...
/**
 * =========================================================
 * PARTE 6 — TELEGRAM PARA APROBAR PAGOS
 * =========================================================
 */

const MD20_TELEGRAM_PAGOS={
  TOKEN:'TELEGRAM_BOT_TOKEN_MD20',
  CHATS:'TELEGRAM_ADMIN_CHAT_IDS_MD20',
  SECRETO:'TELEGRAM_WEBHOOK_SECRET_MD20'
};

function obtenerConfiguracionTelegramPagosMD20_(){
  const propiedades=PropertiesService.getScriptProperties();
  const token=String(propiedades.getProperty(MD20_TELEGRAM_PAGOS.TOKEN)||'').trim();
  const chats=obtenerChatsTelegramPagosMD20_();
  const secreto=String(propiedades.getProperty(MD20_TELEGRAM_PAGOS.SECRETO)||'').trim();
  const url=ScriptApp.getService().getUrl()||'';

  return {
    tokenConfigurado:Boolean(token),
    chatIds:chats,
    webhookSecretoConfigurado:Boolean(secreto),
    urlImplementacion:url,
    webhookUrl:url&&secreto
      ? url+'?telegram='+encodeURIComponent(secreto)
      : '',
    listo:Boolean(token&&chats.length&&secreto&&url)
  };
}

function guardarConfiguracionTelegramPagosMD20_(registro){
  const token=String(registro.token||'').trim();
  const chats=normalizarChatsTelegramPagosMD20_(registro.chatIds);
  let secreto=String(registro.secreto||'').trim();

  const propiedades=PropertiesService.getScriptProperties();

  if(token){
    if(!/^\d+:[A-Za-z0-9_-]{20,}$/.test(token)){
      throw new Error('El token del bot no tiene el formato correcto.');
    }
    propiedades.setProperty(MD20_TELEGRAM_PAGOS.TOKEN,token);
  }

  if(chats.length){
    propiedades.setProperty(
      MD20_TELEGRAM_PAGOS.CHATS,
      chats.join(',')
    );
  }
  ...
  if(!secreto){
    secreto=String(
      propiedades.getProperty(MD20_TELEGRAM_PAGOS.SECRETO)||''
    ).trim();
  }

  if(!secreto){
    secreto=Utilities.getUuid().replace(/-/g,'')+
      Utilities.getUuid().replace(/-/g,'').slice(0,16);
  }

  propiedades.setProperty(
    MD20_TELEGRAM_PAGOS.SECRETO,
    secreto
  );

  return obtenerConfiguracionTelegramPagosMD20_();
}

function normalizarChatsTelegramPagosMD20_(valor){
...
function obtenerChatsTelegramPagosMD20_(){
  const valor=PropertiesService
    .getScriptProperties()
    .getProperty(MD20_TELEGRAM_PAGOS.CHATS);

  return normalizarChatsTelegramPagosMD20_(valor);
}

function obtenerTokenTelegramPagosMD20_(){
  const token=String(
    PropertiesService
      .getScriptProperties()
      .getProperty(MD20_TELEGRAM_PAGOS.TOKEN)||''
  ).trim();

  if(!token){
    throw new Error('El token del bot de Telegram no está configurado.');
  }

  return token;
}

function apiTelegramPagosMD20_(metodo,datos){
  const token=obtenerTokenTelegramPagosMD20_();
  ...
  try{
    contenido=JSON.parse(respuesta.getContentText()||'{}');
  }catch(error){
    contenido={ok:false,description:respuesta.getContentText()};
  }

  if(codigo<200||codigo>=300||!contenido.ok){
    throw new Error(
      contenido.description||
      'Telegram respondió con el código '+codigo+'.'
    );
  }
...
function probarTelegramPagosMD20_(){
  const chats=obtenerChatsTelegramPagosMD20_();

  if(!chats.length){
    throw new Error('Configura por lo menos un Chat ID administrador.');
  }

  let enviados=0;

  chats.forEach(chatId=>{
    apiTelegramPagosMD20_('sendMessage',{
      chat_id:chatId,
      text:
        '✅ *Mundo Digital 2.0 conectado*\n\n'+
        'Las alertas de pagos ya pueden llegar a este chat.',
      parse_mode:'Markdown'
    });
    enviados++;
  });
...
function registrarWebhookTelegramPagosMD20_(){
  const configuracion=obtenerConfiguracionTelegramPagosMD20_();

  if(!configuracion.urlImplementacion){
    throw new Error(
      'Apps Script todavía no tiene una implementación web activa.'
    );
  }

  if(!configuracion.webhookUrl){
    throw new Error(
      'Primero guarda la configuración de Telegram.'
    );
  }

  const resultado=apiTelegramPagosMD20_('setWebhook',{
    url:configuracion.webhookUrl,
    allowed_updates:[
      'message',
      'callback_query'
    ],
  ...
  return {
    ok:true,
    mensaje:'Webhook de Telegram registrado correctamente.',
    resultado:resultado,
    webhookUrl:configuracion.webhookUrl
  };
}

function escaparHtmlTelegramMD20_(valor){
  return String(valor??'')
    .replace(/&/g,'&amp;')
    .replace(/</g,'&lt;')
    .replace(/>/g,'&gt;');
...
function dineroTelegramMD20_(valor,moneda){
  return Number(valor||0).toFixed(2)+' '+String(moneda||'USD');
}

function buscarAlertaPagoTelegramMD20_(pagoId){
  return listarAlertasPagosTiendaMD20_().find(
    alerta=>String(alerta.pagoId)===String(pagoId)
  )||null;
}

function alertasPorPedidoTelegramMD20_(pedido){
  const ventasIds=Array.isArray(pedido.ventasIds)
    ? pedido.ventasIds
    : [];

  return listarAlertasPagosTiendaMD20_().filter(
    alerta=>ventasIds.includes(alerta.ventaId)
  );
}

function textoAlertaTelegramMD20_(alerta){
  const productos=Array.isArray(alerta.productos)&&alerta.productos.length
    ? alerta.productos.map(p=>p.nombre).join(', ')
    : alerta.productoNombre||'Producto digital';

  const montoBs=Number(alerta.totalBs||0)>0
    ? '\n💵 <b>Monto en Bs:</b> '+
      Number(alerta.totalBs).toFixed(2)+' Bs'
    : '';

  return (
    '💳 <b>NUEVO PAGO EN REVISIÓN</b>\n\n'+
    '🧾 <b>Pedido:</b> '+
      escaparHtmlTelegramMD20_(alerta.numeroPedido||alerta.numeroVenta)+'\n'+
    '👤 <b>Cliente:</b> '+
      escaparHtmlTelegramMD20_(alerta.clienteNombre)+'\n'+
    '📦 <b>Producto:</b> '+
      escaparHtmlTelegramMD20_(productos)+'\n'+
    '💰 <b>Monto:</b> '+
      escaparHtmlTelegramMD20_(
        dineroTelegramMD20_(alerta.monto,alerta.moneda)
      )+
      montoBs+'\n'+
    '🏦 <b>Método:</b> '+
      escaparHtmlTelegramMD20_(alerta.metodoPagoNombre)+'\n'+
    '🔢 <b>Referencia:</b> '+
      escaparHtmlTelegramMD20_(alerta.referencia||'Sin referencia')+'\n'+
    '🏛 <b>Origen:</b> '+
      escaparHtmlTelegramMD20_(alerta.bancoOrigen||'No indicado')+'\n'+
    '🛍 <b>Vendedor:</b> '+
      escaparHtmlTelegramMD20_(alerta.vendedor||'ADMIN')+'\n\n'+
    'Selecciona una acción:'
  );
}

function tecladoAlertaTelegramMD20_(alerta){
  const filas=[];

  if(alerta.comprobanteUrl){
    filas.push([
      {
        text:'👁 Ver comprobante',
        url:String(alerta.comprobanteUrl)
      }
    ]);
  }
  ...
  filas.push([
    {
      text:'✅ Confirmar pago',
      callback_data:'pg_ok:'+alerta.pagoId
    },
    {
      text:'❌ Rechazar',
      callback_data:'pg_no:'+alerta.pagoId
    }
  ]);
  ...
  filas.push([
    {
      text:'📄 Ver detalles',
      callback_data:'pg_det:'+alerta.pagoId
    }
  ]);
...
function enviarAlertaPagoTelegramMD20_(alerta){
  const chats=obtenerChatsTelegramPagosMD20_();
  ...
  chats.forEach(chatId=>{
    apiTelegramPagosMD20_('sendMessage',{
      chat_id:chatId,
      text:textoAlertaTelegramMD20_(alerta),
      parse_mode:'HTML',
      disable_web_page_preview:true,
      reply_markup:tecladoAlertaTelegramMD20_(alerta)
    });
    enviados++;
  });
...
function enviarAlertaTelegramPedidoMD20_(pedido){
  const alertas=alertasPorPedidoTelegramMD20_(pedido);

  let enviados=0;

  alertas.forEach(alerta=>{
    enviados+=enviarAlertaPagoTelegramMD20_(alerta).enviados;
  });
...
function enviarAlertasTelegramPendientesMD20_(){
  const alertas=listarAlertasPagosTiendaMD20_();

  if(!alertas.length){
    return {
      ok:true,
      mensaje:'No existen pagos pendientes por enviar.',
      alertas:0,
      enviados:0
    };
  }
  ...
  return {
    ok:true,
    mensaje:'Alertas pendientes enviadas a Telegram.',
    alertas:alertas.length,
    enviados:enviados
  };
}

function procesarWebhookTelegramPagosMD20_(actualizacion,e){
  const propiedades=PropertiesService.getScriptProperties();
  const secretoEsperado=String(
    propiedades.getProperty(MD20_TELEGRAM_PAGOS.SECRETO)||''
  ).trim();

  const secretoRecibido=String(
    e&&e.parameter&&e.parameter.telegram||''
  ).trim();
  ...
  try{
    if(actualizacion.callback_query){
      procesarCallbackTelegramPagoMD20_(
        actualizacion.callback_query
      );
    }else if(actualizacion.message){
      procesarMensajeTelegramPagoMD20_(
...
function esChatAdministradorTelegramMD20_(chatId){
  return obtenerChatsTelegramPagosMD20_().includes(
    String(chatId)
  );
}

function procesarMensajeTelegramPagoMD20_(mensaje){
  const chatId=String(
    mensaje&&mensaje.chat&&mensaje.chat.id||''
  );

  if(!esChatAdministradorTelegramMD20_(chatId)){
    return;
  }

  const texto=String(mensaje.text||'').trim();

  if(['/start','/inicio'].includes(texto.toLowerCase())){
    apiTelegramPagosMD20_('sendMessage',{
      chat_id:chatId,
      text:
        '👋 <b>Mundo Digital 2.0</b>\n\n'+
        'Comandos disponibles:\n'+
        '/pagos — ver pagos pendientes\n'+
        '/rechazar PAGO_ID motivo — rechazar con motivo personalizado',
      parse_mode:'HTML'
    });
    return;
  }

  if(texto.toLowerCase()==='/pagos'){
    const alertas=listarAlertasPagosTiendaMD20_();

    if(!alertas.length){
      apiTelegramPagosMD20_('sendMessage',{
        chat_id:chatId,
        text:'✅ No hay pagos pendientes por revisar.'
      });
      return;
    }
  ...
  const rechazo=texto.match(/^\/rechazar\s+(\S+)\s+(.+)$/i);

  if(rechazo){
    const pagoId=rechazo[1];
    const motivo=rechazo[2].trim();

    const resultado=resolverAlertaPagoTiendaMD20_({
      pagoId:pagoId,
      decision:'RECHAZAR',
      motivo:motivo,
      administrador:'TELEGRAM:'+chatId
    });
...
function procesarCallbackTelegramPagoMD20_(callback){
  const chatId=String(
    callback&&callback.message&&callback.message.chat&&
    callback.message.chat.id||''
  );

  if(!esChatAdministradorTelegramMD20_(chatId)){
    apiTelegramPagosMD20_('answerCallbackQuery',{
  ...
  const datos=String(callback.data||'');
  const partes=datos.split(':');
  const accion=partes[0];
  const pagoId=partes.slice(1).join(':');
  ...
  if(accion==='pg_det'){
    const alerta=buscarAlertaPagoTelegramMD20_(pagoId);
  ...
  if(accion==='pg_no'){
    apiTelegramPagosMD20_('answerCallbackQuery',{
      callback_query_id:callback.id,
      text:'Selecciona el motivo del rechazo.'
    });

    apiTelegramPagosMD20_('editMessageReplyMarkup',{
      chat_id:chatId,
      ...
      reply_markup:{
        inline_keyboard:[
          [
            {
              text:'Comprobante ilegible',
              callback_data:'pg_r_il:'+pagoId
            }
          ],
          [
            {
              text:'Monto incorrecto',
              callback_data:'pg_r_mo:'+pagoId
            }
          ],
          [
            {
              text:'Referencia no encontrada',
              callback_data:'pg_r_re:'+pagoId
            }
          ],
          [
            {
              text:'Cancelar',
              callback_data:'pg_det:'+pagoId
            }
          ]
        ]
      }
  ...
  const motivos={
    pg_r_il:'Comprobante ilegible',
    pg_r_mo:'Monto incorrecto',
    pg_r_re:'Referencia no encontrada'
  };
  ...
  if(accion==='pg_ok'){
    decision='CONFIRMAR';
  }else if(Object.prototype.hasOwnProperty.call(motivos,accion)){
    decision='RECHAZAR';
  ...
  try{
    ...
    const icono=decision==='CONFIRMAR'?'✅':'❌';

    apiTelegramPagosMD20_('editMessageText',{
      chat_id:chatId,
      message_id:callback.message.message_id,
      text:
        callback.message.text+
        '\n\n'+icono+' <b>'+resultado.mensaje+'</b>',
      parse_mode:'HTML',
      disable_web_page_preview:true,
      reply_markup:{
        inline_keyboard:[]
 ...
 * =========================================================
 * MUNDO DIGITAL 2.0
 * TELEGRAM PARA REVENDEDORES — FASE 6B
 * =========================================================
 */

const MD20_TELEGRAM_REVENDEDORES={
  TOKEN:'TELEGRAM_REVENDEDORES_BOT_TOKEN_MD20',
  USERNAME:'TELEGRAM_REVENDEDORES_BOT_USERNAME_MD20',
  SECRETO:'TELEGRAM_REVENDEDORES_WEBHOOK_SECRET_MD20',
  HOJA_USUARIOS:'TELEGRAM_USUARIOS',
  HOJA_MENSAJES:'TELEGRAM_MENSAJES',
  HOJA_ACCIONES:'TELEGRAM_ACCIONES'
};

function prepararTelegramRevendedoresMD20(){
  const libro=SpreadsheetApp.getActiveSpreadsheet();

  prepararHojaTelegramMD20_(
    libro,
    MD20_TELEGRAM_REVENDEDORES.HOJA_USUARIOS,
    [
      'VINCULACION_ID','TIPO_USUARIO','USUARIO_ID','CHAT_ID',
      'USERNAME','NOMBRE_TELEGRAM','TOKEN_VINCULACION','ESTADO',
      'FECHA_VINCULACION','ULTIMA_INTERACCION','CREADO_EN',
      'ACTUALIZADO_EN'
    ],
    '#7B1FA2'
  );
  ...
  prepararHojaTelegramMD20_(
    libro,
    MD20_TELEGRAM_REVENDEDORES.HOJA_MENSAJES,
    [
      'MENSAJE_ID','CHAT_ID','TIPO_USUARIO','USUARIO_ID',
      'TIPO_MENSAJE','TEXTO','ESTADO_ENVIO',
      'TELEGRAM_MESSAGE_ID','ERROR','FECHA'
    ],
    '#512DA8'
  );
  ...
  prepararHojaTelegramMD20_(
    libro,
    MD20_TELEGRAM_REVENDEDORES.HOJA_ACCIONES,
    [
      'ACCION_ID','CHAT_ID','TIPO_USUARIO','USUARIO_ID',
      'ACCION','ENTIDAD','ENTIDAD_ID','RESULTADO','DETALLE','FECHA'
    ],
    '#303F9F'
  );
  ...
  console.log(
    'Telegram para revendedores preparado correctamente.'
  );

  return {
    ok:true,
    mensaje:'Telegram para revendedores preparado correctamente.'
  };
}

function prepararHojaTelegramMD20_(libro,nombre,encabezados,color){
  let hoja=libro.getSheetByName(nombre);
  ...
  encabezados.forEach((encabezado,indice)=>{
    ...
    if(['TEXTO','DETALLE'].includes(encabezado))ancho=330;
    if(['TOKEN_VINCULACION'].includes(encabezado))ancho=300;
    if(['NOMBRE_TELEGRAM','USERNAME'].includes(encabezado))ancho=210;
    if(['CREADO_EN','ACTUALIZADO_EN','FECHA_VINCULACION','ULTIMA_INTERACCION','FECHA'].includes(encabezado))ancho=175;

    hoja.setColumnWidth(indice+1,ancho);
  });
}

function obtenerConfiguracionTelegramRevendedoresMD20_(){
  const propiedades=PropertiesService.getScriptProperties();
  const token=String(
    propiedades.getProperty(MD20_TELEGRAM_REVENDEDORES.TOKEN)||''
  ).trim();
  const username=String(
    propiedades.getProperty(MD20_TELEGRAM_REVENDEDORES.USERNAME)||''
  ).trim().replace(/^@/,'');
  const secreto=String(
    propiedades.getProperty(MD20_TELEGRAM_REVENDEDORES.SECRETO)||''
  ).trim();
  const url=ScriptApp.getService().getUrl()||'';
  ...
  return {
    ...
    urlImplementacion:url,
    webhookUrl:url&&secreto
      ? url+'?telegram_revendedores='+encodeURIComponent(secreto)
      : '',
    listo:Boolean(token&&username&&secreto&&url)
  };
}

function guardarConfiguracionTelegramRevendedoresMD20_(registro){
  const token=String(registro.token||'').trim();
  let secreto=String(registro.secreto||'').trim();

  if(!token){
    throw new Error('Escribe el token del bot de revendedores.');
  }
  ...
  const propiedades=PropertiesService.getScriptProperties();
  propiedades.setProperty(
    MD20_TELEGRAM_REVENDEDORES.TOKEN,
    token
  );
  ...
  if(!secreto){
    secreto=String(
      propiedades.getProperty(
        MD20_TELEGRAM_REVENDEDORES.SECRETO
      )||''
    ).trim();
  }
  ...
  const identidad=apiTelegramRevendedoresMD20_('getMe',{});
  const username=String(identidad.username||'').trim();

  if(!username){
    throw new Error('Telegram no devolvió el usuario del bot.');
  }

  propiedades.setProperty(
    MD20_TELEGRAM_REVENDEDORES.USERNAME,
    username
  );

  return obtenerConfiguracionTelegramRevendedoresMD20_();
}

function obtenerTokenTelegramRevendedoresMD20_(){
  const token=String(
    PropertiesService
      .getScriptProperties()
      .getProperty(MD20_TELEGRAM_REVENDEDORES.TOKEN)||''
  ).trim();

  if(!token){
    throw new Error(
      'El bot de revendedores todavía no está configurado.'
    );
  }
...
function apiTelegramRevendedoresMD20_(metodo,datos){
  const token=obtenerTokenTelegramRevendedoresMD20_();
  const url='https://api.telegram.org/bot'+token+'/'+metodo;
  ...
  try{
    ...
    contenido={
      ok:false,
      description:respuesta.getContentText()
    };
  }

  if(
    respuesta.getResponseCode()<200||
    respuesta.getResponseCode()>=300||
    !contenido.ok
  ){
    throw new Error(
      contenido.description||
      'Telegram no pudo completar la operación.'
    );
  }
...
/**
 * Ejecutar inmediatamente si Telegram está repitiendo mensajes.
 * Detiene el webhook y elimina todos los updates pendientes.
 */
function detenerBucleTelegramRevendedoresMD20(){
  try{
    apiTelegramRevendedoresMD20_('deleteWebhook',{
    ...
    PropertiesService
      .getScriptProperties()
      .deleteProperty(
        'TELEGRAM_REVENDEDORES_ULTIMO_UPDATE_ID_MD20'
      );
  }

  console.log(
    'Webhook detenido y actualizaciones pendientes eliminadas.'
  );
...
/**
 * Limpia updates pendientes y vuelve a registrar el webhook actual.
 */
function reactivarWebhookTelegramRevendedoresMD20(){
  detenerBucleTelegramRevendedoresMD20();
  Utilities.sleep(1000);
  return registrarWebhookTelegramRevendedoresMD20_();
}

function registrarWebhookTelegramRevendedoresMD20_(){
  const configuracion=
    obtenerConfiguracionTelegramRevendedoresMD20_();

  if(!configuracion.listo){
    throw new Error(
      'Guarda primero el token del bot y actualiza la implementación.'
    );
  }

  // Elimina cualquier webhook anterior antes de registrar el nuevo.
  apiTelegramRevendedoresMD20_('deleteWebhook',{
    drop_pending_updates:true
  });
  ...
  const resultado=apiTelegramRevendedoresMD20_('setWebhook',{
    url:configuracion.webhookUrl,
    allowed_updates:['message','callback_query'],
    drop_pending_updates:true
  });

  const info=apiTelegramRevendedoresMD20_('getWebhookInfo',{});

  const urlTelegram=String(info.url||'').trim();
  const urlEsperada=String(configuracion.webhookUrl||'').trim();

  if(!urlTelegram||urlTelegram!==urlEsperada){
    throw new Error(
      'Telegram no guardó el webhook correcto. URL actual: '+
      (urlTelegram||'vacía')
    );
  }

  // Reinicia el control para que los nuevos updates no sean descartados.
  PropertiesService
    .getScriptProperties()
    .deleteProperty('TELEGRAM_REVENDEDORES_ULTIMO_UPDATE_ID_MD20');

  return {
    ok:true,
    mensaje:'Webhook registrado y verificado correctamente.',
    webhook:{
      url:urlTelegram,
      pendientes:Number(info.pending_update_count||0),
      ultimoError:String(info.last_error_message||''),
      fechaUltimoError:info.last_error_date
        ? new Date(Number(info.last_error_date)*1000)
        : ''
    }
  };
}

function diagnosticarWebhookTelegramRevendedoresMD20_(){
  const configuracion=
    obtenerConfiguracionTelegramRevendedoresMD20_();
  ...
  return {
    ...
    telegram:{
      url:String(info.url||''),
      tieneCertificado:Boolean(info.has_custom_certificate),
      actualizacionesPendientes:Number(info.pending_update_count||0),
      ultimoErrorFecha:info.last_error_date
        ? fechaHoraApi_(new Date(Number(info.last_error_date)*1000))
        : '',
      ultimoErrorMensaje:String(info.last_error_message||''),
      conexionesMaximas:Number(info.max_connections||0),
      actualizacionesPermitidas:Array.isArray(info.allowed_updates)
        ? info.allowed_updates
        : []
    },
    coincide:
      String(info.url||'')===
      String(configuracion.webhookUrl||'')
  };
}

function probarTelegramRevendedoresMD20_(){
  const identidad=apiTelegramRevendedoresMD20_('getMe',{});
  ...
  return {
    ...
    mensaje:
      ...
      String(identidad.username||'')
  };
}

function generarEnlaceTelegramRevendedorMD20_(revendedorId){
  const configuracion=
    obtenerConfiguracionTelegramRevendedoresMD20_();

  if(!configuracion.botUsername){
    throw new Error(
      'Primero configura el bot de revendedores.'
    );
  }

  const revendedor=buscarRevendedorTelegramMD20_(revendedorId);
  ...
  const hoja=SpreadsheetApp
    .getActiveSpreadsheet()
    .getSheetByName(MD20_TELEGRAM_REVENDEDORES.HOJA_USUARIOS);

  if(!hoja){
    throw new Error(
      'Primero ejecuta prepararTelegramRevendedoresMD20.'
    );
  }

  const existente=buscarVinculacionTelegramPorUsuarioMD20_(
    revendedorId
  );

  let token=existente
    ? String(existente.tokenVinculacion||'').trim()
    : '';

  if(!token){
    token=
      'REV'+
      Utilities.getUuid().replace(/-/g,'').slice(0,28);
  }

  const ahora=new Date();
  const vinculacionId=existente
    ? existente.vinculacionId
    : 'TGV-'+Utilities.getUuid()
        .replace(/-/g,'')
        .slice(0,12)
        .toUpperCase();
  ...
  const fila=existente
    ? existente.fila
    : primeraFilaLibreTelegramMD20_(hoja);

  hoja.getRange(fila,1,1,12).setValues([[
    vinculacionId,
    'REVENDEDOR',
    revendedorId,
    existente?existente.chatId:'',
    existente?existente.username:'',
    existente?existente.nombreTelegram:'',
    token,
    existente&&existente.chatId?'VINCULADO':'PENDIENTE',
    existente?existente.fechaVinculacion:'',
    existente?existente.ultimaInteraccion:'',
  ...
  return {
    ...
    estado:existente&&existente.chatId
      ? 'VINCULADO'
      : 'PENDIENTE',
    enlace:
      'https://t.me/'+
      configuracion.botUsername+
      '?start='+
      encodeURIComponent(token),
    botUsername:'@'+configuracion.botUsername
  };
}

function desvincularTelegramRevendedorMD20_(revendedorId){
  const vinculacion=
    buscarVinculacionTelegramPorUsuarioMD20_(revendedorId);

  if(!vinculacion){
    throw new Error(
      'El revendedor no tiene una vinculación de Telegram.'
    );
  }
  ...
  hoja.getRange(vinculacion.fila,4,1,3).clearContent();
  hoja.getRange(vinculacion.fila,8).setValue('PENDIENTE');
  hoja.getRange(vinculacion.fila,9,1,2).clearContent();
  hoja.getRange(vinculacion.fila,12).setValue(new Date());

  SpreadsheetApp.flush();

  return generarEnlaceTelegramRevendedorMD20_(revendedorId);
}

function enviarPruebaTelegramRevendedorMD20_(revendedorId){
  const vinculacion=
    buscarVinculacionTelegramPorUsuarioMD20_(revendedorId);

  if(!vinculacion||!vinculacion.chatId){
    throw new Error(
      'El revendedor todavía no ha conectado su Telegram.'
    );
  }

  const revendedor=buscarRevendedorTelegramMD20_(revendedorId);

  apiTelegramRevendedoresMD20_('sendMessage',{
    chat_id:vinculacion.chatId,
    text:
      '✅ *Mundo Digital 2.0 conectado*\n\n'+
      'Hola '+(revendedor?revendedor.nombre:'revendedor')+'.\n'+
      'Tu Telegram ya está relacionado con tu panel de ventas.',
    parse_mode:'Markdown'
  });

  registrarMensajeTelegramRevendedorMD20_({
    chatId:vinculacion.chatId,
    usuarioId:revendedorId,
    tipo:'PRUEBA',
    texto:'Mensaje de prueba enviado.',
    estado:'ENVIADO'
  });

  return {
    ok:true,
    mensaje:'Mensaje de prueba enviado al revendedor.'
  };
}

function listarVinculacionesTelegramRevendedoresMD20_(){
  const revendedores=listarSocios_('REVENDEDOR');

  return revendedores.map(revendedor=>{
    const vinculacion=
      buscarVinculacionTelegramPorUsuarioMD20_(revendedor.id);

    return {
      revendedorId:revendedor.id,
      nombre:
        String(revendedor.nombre||'')+' '+
        String(revendedor.apellido||''),
      whatsapp:revendedor.whatsapp||'',
      estadoRevendedor:revendedor.estado||'ACTIVO',
      tiendaActiva:revendedor.tiendaActiva||'SI',
      telegramEstado:vinculacion
        ? vinculacion.estado
        : 'SIN_CONFIGURAR',
      telegramUsername:vinculacion
        ? vinculacion.username
        : '',
      chatIdProtegido:vinculacion&&vinculacion.chatId
        ? protegerChatIdTelegramMD20_(vinculacion.chatId)
        : '',
      fechaVinculacion:vinculacion
        ? fechaHoraApi_(vinculacion.fechaVinculacion)
        : '',
      tieneToken:Boolean(
        vinculacion&&vinculacion.tokenVinculacion
      )
    };
  });
}
...
function aceptarUpdateTelegramRevendedoresMD20_(updateId){
  ...
  try{
    const propiedades=PropertiesService.getScriptProperties();
    const clave='TELEGRAM_REVENDEDORES_ULTIMO_UPDATE_ID_MD20';
    const ultimo=Number(propiedades.getProperty(clave)||0);

    if(updateId<=ultimo){
      return false;
    }

    propiedades.setProperty(clave,String(updateId));
    return true;
  }finally{
    lock.releaseLock();
  }
...
function procesarWebhookTelegramRevendedoresMD20_(actualizacion,e){
  const secretoRecibido=String(
    e&&e.parameter&&e.parameter.telegram_revendedores||''
  ).trim();
  ...
  const updateId=Number(
    actualizacion&&actualizacion.update_id||0
  );

  if(!aceptarUpdateTelegramRevendedoresMD20_(updateId)){
    return ContentService
      .createTextOutput('OK')
      .setMimeType(ContentService.MimeType.TEXT);
  }
  ...
  try{
    if(actualizacion.message){
      procesarMensajeTelegramRevendedorMD20_(
        actualizacion.message
      );
    }

    if(actualizacion.callback_query){
      procesarCallbackTelegramRevendedorMD20_(
        actualizacion.callback_query
      );
    }
  }catch(error){
    try{
      registrarErrorWebhookTelegramRevendedoresMD20_(
        actualizacion,
        error
      );
    }catch(errorFinal){
      console.error(
        'Error final del webhook: '+
        String(errorFinal&&errorFinal.message||errorFinal)
      );
    }
  }
...
function registrarErrorWebhookTelegramRevendedoresMD20_(
  actualizacion,
  error
){
  const chatId=String(
    actualizacion&&actualizacion.message&&
    actualizacion.message.chat&&
    actualizacion.message.chat.id||
    actualizacion&&actualizacion.callback_query&&
    actualizacion.callback_query.message&&
    actualizacion.callback_query.message.chat&&
  ...
  // Nunca enviar mensajes desde el manejador global de errores.
  // Esto evita ciclos cuando también falla la API de Telegram.
  try{
    registrarAccionTelegramRevendedorMD20_({
      chatId:chatId,
      usuarioId:'',
      accion:'ERROR_WEBHOOK',
      entidad:'TELEGRAM_UPDATE',
      entidadId:String(
        actualizacion&&actualizacion.update_id||''
      ),
      resultado:'ERROR',
      detalle:detalle
    });
    ...
    console.error(
      'No se pudo registrar el error del webhook: '+
      String(errorRegistro&&errorRegistro.message||errorRegistro)
    );
  }

  console.error('Telegram revendedores: '+detalle);
}

function procesarMensajeTelegramRevendedorMD20_(mensaje){
  const chatId=String(
    mensaje&&mensaje.chat&&mensaje.chat.id||''
  );
  const texto=String(mensaje&&mensaje.text||'').trim();

  if(!chatId)return;

  if(texto.startsWith('/start')){
    const token=texto.split(/\s+/)[1]||'';
    vincularTelegramRevendedorMD20_(
      token,
      mensaje
    );
    return;
  ...
  const vinculacion=
    buscarVinculacionTelegramPorChatMD20_(chatId);

  if(!vinculacion){
    apiTelegramRevendedoresMD20_('sendMessage',{
      chat_id:chatId,
      text:
        'Este Telegram todavía no está vinculado.\n\n'+
        'Abre el enlace privado generado por el administrador.'
    });
    return;
  }

  actualizarInteraccionTelegramMD20_(
    vinculacion.fila
  );
  ...
  if(
    ...
    enviarMenuTelegramRevendedorMD20_(
      vinculacion
    );
    return;
  }
  ...
  apiTelegramRevendedoresMD20_('sendMessage',{
    chat_id:chatId,
    text:
      'Usa el menú para consultar tu información.',
    reply_markup:tecladoMenuTelegramRevendedorMD20_()
  });
}

function vincularTelegramRevendedorMD20_(token,mensaje){
  const vinculacion=
    buscarVinculacionTelegramPorTokenMD20_(token);
  ...
  if(!vinculacion){
    apiTelegramRevendedoresMD20_('sendMessage',{
      chat_id:chatId,
      text:
        'El enlace de vinculación no es válido o fue reemplazado.'
    });
    return;
  }

  const otraVinculacion=
    buscarVinculacionTelegramPorChatMD20_(chatId);

  if(
    otraVinculacion&&
    otraVinculacion.usuarioId===vinculacion.usuarioId
  ){
    actualizarInteraccionTelegramMD20_(otraVinculacion.fila);

    apiTelegramRevendedoresMD20_('sendMessage',{
      chat_id:chatId,
      text:
        'Tu Telegram ya está vinculado con este revendedor.\n\n'+
  ...
  if(
    ...
    apiTelegramRevendedoresMD20_('sendMessage',{
      chat_id:chatId,
      text:
        'Este Telegram ya está vinculado con otro revendedor.'
    });
    return;
  }
  ...
  const nombreTelegram=[
    mensaje.from&&mensaje.from.first_name||'',
    mensaje.from&&mensaje.from.last_name||''
  ].filter(Boolean).join(' ');

  hoja.getRange(vinculacion.fila,4,1,6).setValues([[
    chatId,
    String(
      mensaje.from&&mensaje.from.username||''
    ),
    nombreTelegram,
    '',
    'VINCULADO',
  ...
  hoja.getRange(vinculacion.fila,10).setValue(new Date());
  hoja.getRange(vinculacion.fila,12).setValue(new Date());
  ...
  registrarAccionTelegramRevendedorMD20_({
    ...
    entidad:'REVENDEDOR',
    entidadId:vinculacion.usuarioId,
    resultado:'OK',
    detalle:nombreTelegram
  });

  const revendedor=
    buscarRevendedorTelegramMD20_(
      vinculacion.usuarioId
    );
  ...
  apiTelegramRevendedoresMD20_('sendMessage',{
    ...
    text:
      ...
      'Hola '+
      (revendedor?revendedor.nombre:nombreTelegram)+
      '.\n'+
      'Ya puedes consultar la información de tu tienda y tus ventas.',
    parse_mode:'Markdown',
    reply_markup:tecladoMenuTelegramRevendedorMD20_()
  });
}

function tecladoMenuTelegramRevendedorMD20_(){
  return {
    inline_keyboard:[
      [
        {text:'🏠 Mi resumen',callback_data:'rev_resumen'},
        {text:'🏪 Mi tienda',callback_data:'rev_tienda'}
      ],
      [
        {text:'🛒 Mis ventas',callback_data:'rev_ventas'},
        {text:'💳 Mis pagos',callback_data:'rev_pagos'}
      ],
      [
        {text:'👥 Mis clientes',callback_data:'rev_clientes'},
        {text:'💰 Mis ganancias',callback_data:'rev_ganancias'}
      ],
      [
        {text:'💎 Mis diamantes',callback_data:'rev_diamantes'},
        {text:'🆘 Soporte',callback_data:'rev_soporte'}
      ]
    ]
  };
}

function enviarMenuTelegramRevendedorMD20_(vinculacion){
  const revendedor=
    buscarRevendedorTelegramMD20_(
      vinculacion.usuarioId
    );
  ...
  apiTelegramRevendedoresMD20_('sendMessage',{
    ...
    text:
      '🏠 *Mundo Digital 2.0*\n\n'+
      'Revendedor: '+
      (revendedor?revendedor.nombre:vinculacion.usuarioId)+
...
function procesarCallbackTelegramRevendedorMD20_(callback){
  const chatId=String(
  ...
  if(!vinculacion){
    apiTelegramRevendedoresMD20_(
      'answerCallbackQuery',
      {
        callback_query_id:callback.id,
        text:'Telegram no vinculado.',
        show_alert:true
      }
    );
    return;
  ...
  const revendedorId=String(vinculacion.usuarioId||'');
  const revendedor=buscarRevendedorTelegramMD20_(revendedorId);

  try{
    const texto=resolverBotonTelegramRevendedorMD20_(
      data,
      revendedorId,
      revendedor
    );
    ...
    apiTelegramRevendedoresMD20_('sendMessage',{
      chat_id:chatId,
      text:String(texto||'No hay información disponible.'),
      reply_markup:tecladoMenuTelegramRevendedorMD20_()
    });
    ...
    try{
      registrarAccionTelegramRevendedorMD20_({
        ...
        entidadId:'',
        resultado:'ERROR',
        detalle:mensajeError.slice(0,1000)
      });
    }catch(errorRegistro){
      console.error(
        'No se pudo registrar el error del botón: '+
        String(errorRegistro&&errorRegistro.message||errorRegistro)
      );
    }

    // Solo se intenta una respuesta y jamás se relanza su error.
    try{
      apiTelegramRevendedoresMD20_('sendMessage',{
        chat_id:chatId,
        text:
          'Esta sección presentó un error y quedó registrada para revisión.\n\n'+
          'Botón: '+nombreBotonTelegramRevendedorMD20_(data),
        reply_markup:tecladoMenuTelegramRevendedorMD20_()
      });
    }catch(errorEnvio){
      console.error(
        'No se pudo enviar el aviso del botón: '+
        String(errorEnvio&&errorEnvio.message||errorEnvio)
      );
    }
  }
}

function resolverBotonTelegramRevendedorMD20_(
  data,
  revendedorId,
  revendedor
){
  if(data==='rev_resumen'){
    return construirResumenTelegramRevendedorMD20_(
      revendedorId,
      revendedor
    );
  }
  ...
  if(data==='rev_tienda'){
    return construirTiendaTelegramRevendedorMD20_(
      revendedorId,
      revendedor
    );
  }
  ...
  if(data==='rev_ventas'){
    return construirVentasTelegramRevendedorMD20_(
      revendedorId
    );
  }

  if(data==='rev_pagos'){
    return construirPagosTelegramRevendedorMD20_(
      revendedorId
    );
  }

  if(data==='rev_clientes'){
    return construirClientesTelegramRevendedorMD20_(
      revendedorId
    );
  }

  if(data==='rev_ganancias'){
    return construirGananciasTelegramRevendedorMD20_(
      revendedorId
    );
  }

  if(data==='rev_diamantes'){
    return (
      '💎 MIS DIAMANTES\n\n'+
      'El sistema de diamantes se conectará después de terminar '+
      'las ventas, pagos y ganancias del revendedor.'
    );
  }
  ...
  if(data==='rev_soporte'){
    ...
    return (
      ...
      String(
        configuracion&&configuracion.nombreNegocio||
        'Mundo Digital 2.0'
      )+
      '.'
    );
...
function nombreBotonTelegramRevendedorMD20_(data){
  const nombres={
    rev_resumen:'Mi resumen',
    rev_tienda:'Mi tienda',
    rev_ventas:'Mis ventas',
    rev_pagos:'Mis pagos',
    rev_clientes:'Mis clientes',
    rev_ganancias:'Mis ganancias',
    rev_diamantes:'Mis diamantes',
    rev_soporte:'Soporte'
  };

  return nombres[data]||data||'Opción desconocida';
}

function ventasTelegramRevendedorMD20_(revendedorId){
  const id=String(revendedorId||'').trim();
  let registros=[];

  try{
    registros=listarVentasMD20_();
  }catch(error){
    throw new Error(
      'No se pudieron consultar las ventas: '+error.message
    );
  }

  return (Array.isArray(registros)?registros:[]).filter(venta=>{
    const notas=String(venta.notas||'');
    const canal=String(venta.canalVenta||'').toUpperCase();

    return (
      notas.includes('VENDEDOR: '+id)||
      notas.includes('VENDEDOR_ID: '+id)||
      notas.includes('REVENDEDOR: '+id)||
      (
        canal==='TIENDA_REVENDEDOR'&&
        notas.includes(id)
      )
    );
  });
...
function pagosTelegramRevendedorMD20_(revendedorId){
  const ventas=ventasTelegramRevendedorMD20_(revendedorId);
  const ids=new Set(ventas.map(venta=>String(venta.id)));
  let registros=[];

  try{
    registros=listarPagosMD20_();
  }catch(error){
    throw new Error(
      'No se pudieron consultar los pagos: '+error.message
    );
  }

  return (Array.isArray(registros)?registros:[]).filter(
    pago=>ids.has(String(pago.ventaId))
  );
}

function clientesTelegramRevendedorMD20_(revendedorId){
  const ventas=ventasTelegramRevendedorMD20_(revendedorId);
  const clientes={};
  ...
  ventas.forEach(venta=>{
    ...
    if(!clientes[id]){
      clientes[id]={
        id:id,
        nombre:String(venta.clienteNombre||'Cliente'),
        telefono:String(venta.clienteTelefono||''),
        compras:0,
        total:0,
        moneda:String(venta.moneda||'USD')
      };
    }

    clientes[id].compras+=1;
    clientes[id].total+=Number(venta.total||0);
  });
...
function pedidosTelegramRevendedorMD20_(revendedorId){
  const hoja=SpreadsheetApp
    .getActiveSpreadsheet()
    .getSheetByName('PEDIDOS_TIENDA');
  ...
  const indice=nombre=>encabezados.indexOf(nombre);
  const colVendedor=indice('VENDEDOR_ID');

  if(colVendedor<0)return[];

  return hoja
    .getRange(2,1,hoja.getLastRow()-1,hoja.getLastColumn())
    .getValues()
    .filter(fila=>
      String(fila[colVendedor]||'')===String(revendedorId)
    )
    .map(fila=>{
      const valor=nombre=>{
        const i=indice(nombre);
      ...
      return {
        ...
        estadoPedido:String(valor('ESTADO_PEDIDO')||'PENDIENTE'),
        estadoPago:String(valor('ESTADO_PAGO')||'PENDIENTE'),
        creadoEn:valor('CREADO_EN')
      };
    });
}

function construirResumenTelegramRevendedorMD20_(revendedorId,revendedor){
  const ventas=ventasTelegramRevendedorMD20_(revendedorId);
  const pagos=pagosTelegramRevendedorMD20_(revendedorId);
  const clientes=clientesTelegramRevendedorMD20_(revendedorId);
  const pedidos=pedidosTelegramRevendedorMD20_(revendedorId);

  const confirmadas=ventas.filter(
    venta=>String(venta.estadoPago).toUpperCase()==='PAGADO'
  );
  ...
  const totalConfirmado=confirmadas.reduce(
    (total,venta)=>total+Number(venta.total||0),
    0
  );

  const moneda=confirmadas[0]?.moneda||
    ventas[0]?.moneda||
    'USD';
  ...
  return (
    ...
    '\nEstado: '+String(revendedor?.estado||'ACTIVO')+
    '\nTienda: '+
    (
      String(revendedor?.tiendaActiva||'SI')==='NO'
        ? 'APAGADA'
        : 'ACTIVA'
    )+
    '\n\n'+
    '🛒 Ventas registradas: '+ventas.length+
    '\n✅ Ventas pagadas: '+confirmadas.length+
    '\n💳 Pagos pendientes: '+pendientes.length+
    '\n👥 Clientes: '+clientes.length+
    '\n📦 Pedidos web: '+pedidos.length+
    '\n💰 Total confirmado: '+
    formatoDineroTelegramMD20_(totalConfirmado,moneda)
  );
}

function construirTiendaTelegramRevendedorMD20_(revendedorId,revendedor){
  const productos=listarCatalogoPublicoMD20_(revendedorId);
  const pedidos=pedidosTelegramRevendedorMD20_(revendedorId);
  const ventas=ventasTelegramRevendedorMD20_(revendedorId);
  ...
  return (
    '🏪 MI TIENDA\n\n'+
    'Estado: '+estado+
    '\nRevendedor ID: '+revendedorId+
    '\nProductos disponibles: '+productos.length+
    '\nPedidos recibidos: '+pedidos.length+
    '\nVentas confirmadas: '+activas+
    '\n\n'+
    'Enlace personal:\n'+
    'tienda.html?vendedor='+revendedorId+
    '\n\nEl enlace será compartible por Internet cuando publiquemos la página en Vercel.'
  );
}

function construirVentasTelegramRevendedorMD20_(revendedorId){
  const ventas=ventasTelegramRevendedorMD20_(revendedorId)
    .slice(0,8);
  ...
  if(!ventas.length){
    return (
      '🛒 MIS VENTAS\n\n'+
      'Todavía no tienes ventas registradas desde tu tienda.'
    );
  }

  const lineas=['🛒 MIS VENTAS',''];

  ventas.forEach((venta,indice)=>{
    lineas.push(
      (indice+1)+'. '+
      textoSeguroTelegramMD20_(
    ...
    lineas.push(
      'Total: '+
      formatoDineroTelegramMD20_(
        venta.total,
        venta.moneda
      )
    );
    lineas.push(
      'Pago: '+String(venta.estadoPago||'PENDIENTE')
    );
    lineas.push(
      'Entrega: '+String(venta.estadoEntrega||'PENDIENTE')
    );
    lineas.push('');
  });

  lineas.push('Mostrando las últimas '+ventas.length+' ventas.');
  return lineas.join('\n');
}

function construirPagosTelegramRevendedorMD20_(revendedorId){
  const pagos=pagosTelegramRevendedorMD20_(revendedorId)
    .slice(0,8);
  ...
  const lineas=['💳 MIS PAGOS',''];

  pagos.forEach((pago,indice)=>{
    lineas.push(
      (indice+1)+'. '+
      textoSeguroTelegramMD20_(
        pago.numeroVenta||pago.id
      )
    );
    lineas.push(
      'Cliente: '+
    ...
    lineas.push(
      'Método: '+
      textoSeguroTelegramMD20_(
        pago.metodoPagoNombre||pago.metodoPagoId
      )
    );
    lineas.push('Estado: '+String(pago.estado||'PENDIENTE'));
    lineas.push('');
  });
...
function construirClientesTelegramRevendedorMD20_(revendedorId){
  const clientes=clientesTelegramRevendedorMD20_(revendedorId)
    .slice(0,10);
  ...
  if(!clientes.length){
    return (
      '👥 MIS CLIENTES\n\n'+
      'Todavía no tienes clientes asociados con ventas.'
    );
  }

  const lineas=['👥 MIS CLIENTES',''];

  clientes.forEach((cliente,indice)=>{
    lineas.push(
      (indice+1)+'. '+
      textoSeguroTelegramMD20_(cliente.nombre)
    );
    lineas.push(
      'WhatsApp: '+
      textoSeguroTelegramMD20_(
    ...
    lineas.push('Compras: '+cliente.compras);
    lineas.push(
      'Total comprado: '+
      formatoDineroTelegramMD20_(
        cliente.total,
...
function construirGananciasTelegramRevendedorMD20_(revendedorId){
  const ventas=ventasTelegramRevendedorMD20_(revendedorId);
  const pagadas=ventas.filter(
    venta=>String(venta.estadoPago).toUpperCase()==='PAGADO'
  );

  const moneda=pagadas[0]?.moneda||
    ventas[0]?.moneda||
    'USD';

  const totalVendido=pagadas.reduce(
    (total,venta)=>total+Number(venta.total||0),
    0
  );
  ...
  const pendientes=ventas
    .filter(venta=>
      String(venta.estadoPago).toUpperCase()!=='PAGADO'
    )
    .reduce(
      (total,venta)=>total+Number(venta.total||0),
      0
  ...
  return (
    ...
    '\nTotal vendido confirmado: '+
    formatoDineroTelegramMD20_(totalVendido,moneda)+
    '\nVentas pendientes: '+
    formatoDineroTelegramMD20_(pendientes,moneda)+
    '\n\n'+
    'La comisión o ganancia neta se calculará con las reglas de revendedores que configuraremos en la siguiente etapa.'
  );
}

function formatoDineroTelegramMD20_(valor,moneda){
  const numero=Number(valor||0);
  return numero.toFixed(2)+' '+String(moneda||'USD');
}

function textoSeguroTelegramMD20_(valor){
  return String(valor===null||valor===undefined?'':valor)
    .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/g,'')
    .slice(0,500);
}

function buscarRevendedorTelegramMD20_(revendedorId){
  return listarSocios_('REVENDEDOR').find(
    revendedor=>
      String(revendedor.id)===String(revendedorId)
  )||null;
}

function leerVinculacionesTelegramMD20_(){
  const hoja=SpreadsheetApp
    .getActiveSpreadsheet()
    .getSheetByName(
      MD20_TELEGRAM_REVENDEDORES.HOJA_USUARIOS
  ...
  return hoja
    ...
    .map((fila,indice)=>({
      fila:indice+2,
      vinculacionId:String(fila[0]||''),
      tipoUsuario:String(fila[1]||''),
      usuarioId:String(fila[2]||''),
      chatId:String(fila[3]||''),
      username:String(fila[4]||''),
      nombreTelegram:String(fila[5]||''),
      tokenVinculacion:String(fila[6]||''),
      estado:String(fila[7]||'PENDIENTE'),
      fechaVinculacion:fila[8]||'',
      ultimaInteraccion:fila[9]||'',
      creadoEn:fila[10]||'',
      actualizadoEn:fila[11]||''
    }))
    .filter(item=>item.vinculacionId);
}

function buscarVinculacionTelegramPorUsuarioMD20_(usuarioId){
  return leerVinculacionesTelegramMD20_().find(
    item=>item.usuarioId===String(usuarioId)
  )||null;
}

function buscarVinculacionTelegramPorTokenMD20_(token){
  return leerVinculacionesTelegramMD20_().find(
    item=>
      item.tokenVinculacion&&
      item.tokenVinculacion===String(token||'')
  )||null;
}

function buscarVinculacionTelegramPorChatMD20_(chatId){
  return leerVinculacionesTelegramMD20_().find(
    item=>
      item.chatId&&
...
function primeraFilaLibreTelegramMD20_(hoja){
  const valores=hoja
    .getRange(2,1,hoja.getMaxRows()-1,1)
    .getDisplayValues()
    .flat();
...
function actualizarInteraccionTelegramMD20_(fila){
  const hoja=SpreadsheetApp
    .getActiveSpreadsheet()
    .getSheetByName(
      MD20_TELEGRAM_REVENDEDORES.HOJA_USUARIOS
...
function protegerChatIdTelegramMD20_(chatId){
  const valor=String(chatId||'');
  if(valor.length<=4)return '••••';
  return '••••••'+valor.slice(-4);
}

function registrarMensajeTelegramRevendedorMD20_(datos){
  const hoja=SpreadsheetApp
    .getActiveSpreadsheet()
    .getSheetByName(
      MD20_TELEGRAM_REVENDEDORES.HOJA_MENSAJES
...
/**
 * Ejecutar una sola vez después de instalar esta corrección.
 * Limpia el último update procesado para iniciar el control desde cero.
 */
function reiniciarControlRepeticionesTelegramRevendedoresMD20(){
  PropertiesService
    .getScriptProperties()
    .deleteProperty('TELEGRAM_REVENDEDORES_ULTIMO_UPDATE_ID_MD20');

  console.log(
    'Control de repeticiones de Telegram reiniciado correctamente.'
  );

  return {
    ok:true,
    mensaje:'Control de repeticiones reiniciado correctamente.'
  };
}

function registrarAccionTelegramRevendedorMD20_(datos){
  const hoja=SpreadsheetApp
    .getActiveSpreadsheet()
    .getSheetByName(
      MD20_TELEGRAM_REVENDEDORES.HOJA_ACCIONES
